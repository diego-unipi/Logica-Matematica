...
Dal lemma segue in particolare che se $M \models \mathsf{Q}$ e $a_1,\ldots,a_k \in \NN$, allora $M \models \psi(\ol{a_1},\ldots,\ol{a_k})$ se e solo se $\mathsf{Q} \vdash \psi(\ol{a_1},\ldots,\ol{a_k})$.
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        Essendo la formula chiusa allora possiamo mostrare equivalentemente che vale $\mathsf{Q} \models \forall y \, (\varphi(\ol{x_1}, \dots, \ol{x_k}, y) \to y = \ol{f(x_1, \dots, x_k)})$. Procediamo per assurdo,
        allora esiste $M \models \mathsf{Q}$ tale che per ogni $y \in M$ vale $M \models \varphi(\ol{x_1}, \dots, \ol{x_k}, y)$ e $y \ne \{\ldots\}_M \ol{f(x_1, \dots, x_k)}$.\\
        ...
        \begin{itemize}
            ...
            si ha che $y \in M$ è standard e che possiamo prendere $z_1, \dots, z_l$ standard, per cui, sostituendo i numerali otteniamo che:
            \[ M \models \psi(\ol{x_1}, \dots, \ol{x_k}, y, z_1, \dots, z_l) \]
            ma, per il lemma usato all'inizio, equivale al fatto che $y = f(x_1, \dots, x_k)$, ovvero $M \models y = \ol{f(x_1, \dots, x_k)}$, assurdo.
            ...
            ponendo $t = \{\ldots\}_M \ol{f(x_1, \dots, x_k)}$ che è standard, e fissando dal lemma iniziale che $z_1, \dots, z_l$ standard per i quali $\mathsf{Q} \vdash \psi(\ol{x_1}, \dots, \ol{x_k}, t, z_1, \dots, z_l)$, qualsiasi sia $t$ standard,
            allora si ottiene vero l'antecedente di $\theta_2$, e quindi $M \models y = \ol{f(x_1, \dots, x_k)}$, assurdo.
        \end{itemize}
        ...
        Ossia, per El$_\forall$, $\mathsf{Q} \vdash \varphi(\ol{x_1}, \dots, \ol{x_k}, \ol{f(x_1, \dots, x_k)})$. Dall'ipotesi iniziale $f(x_1, \dots, x_k) = y$ equivale a $\mathsf{Q} \vdash \exists z_1 \dots \exists z_l \, \psi(\ol{x_1}, \dots, \ol{x_k}, \ol{f(x_1, \dots, x_k)}, z_1, \dots, z_l)$,
        fissando $z_1, \dots, z_l$ tali che $\mathsf{Q} \vdash \psi(\ol{x_1}, \dots, \ol{x_k}, \ol{f(x_1, \dots, x_k)}, \ol{z_1}, \dots, \ol{z_l})$, scegliamo:
        \[ N := \max\{f(x_1, \dots, x_k), z_1, \dots, z_l\}\]
        in ogni modello $M \models \mathsf{Q}$. Fissato quindi $M \models \mathsf{Q}$, i numeri $N$ e $f(x_1, \dots, x_k)$ soddisfano $\theta_1$ e $\theta_2$ per costruzione, infatti: per quanto riguarda $\theta_1$ i quantificatori esistenziali:
        \[ M \models \exists z_1 \le \ol N \ldots \exists z_l \le \ol N \, (y \le \ol N \land \psi(\ol{x_1}, \dots, \ol{x_k}, y, z_1, \dots, z_l) \]
        sono semplicemente testimoniati dai numeri $z_1,\ldots,z_n$ fissati in precedenza. Per quanto riguarda $\theta_2$ dobbiamo verificare che:
        \[ M \models \forall z_1 \le N \dots \forall z_l \le N \, \forall t \le N \, (\psi(\ol{x_1}, \dots, \ol{x_k}, t, z_1, \dots, z_l) \to t = \ol{f(x_1, \dots, x_k)}) \]
        ossia, fissati $z_1, \dots, z_l,t \in M$ tali che:
        \[ M \models z_1 \le\ol N \land \ldots \land z_l \le \ol N \land t \le \ol N \land \psi(\ol{x_1}, \dots, \ol{x_k}, t, z_1, \dots, z_l) \]
        dobbiamo avere necessariamente $M \models t = \ol{f(x_1, \dots, x_k)}$. Però, per un lemma, le disuguaglianze appena scritte implicano che $z_1, \dots, z_l$ e $t$ sono standard, per cui dalla premessa si arriva, sostituendo, a:
        \[ M \models \psi(\ol{x_1}, \dots, \ol{x_k}, t, z_1, \dots, z_l) \]
        e per l'ipotesi iniziale sulla rappresentazione di $f$ in $\mathsf{Q}$, si ottiene $t = f(x_1, \dots, x_k)$.
    \end{itemize}
\end{proof}
...
\begin{remark}[Ricorsivamente assiomatizzabile $\implies$ semidecidibile]
    ...
    \[ \{\ulcorner\sigma\urcorner : T \vdash \sigma\} = \{\ulcorner\sigma\urcorner : \exists d \, \mathsf{Dim}_T(d,\ulcorner\sigma\urcorner)\} \]
