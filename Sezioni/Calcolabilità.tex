...
\begin{definition}[Teoria ricorsivamente assiomatizzabile]
    Una teoria $T$ è ricorsivamente assiomatizzabile se:
    \[ T = \{\sigma : P(\ulcorner\sigma\urcorner) = 0 \} \]
    con $P$ una funzione ricorsiva totale.
\end{definition}

In realtà, per via di un trucco, possiamo sempre supporre che l'insieme di assiomi sia decidibile.

\begin{exercise}[Ogni teoria semidecidibile è equivalente ad una decidibile ]
    Sia $T := \{\varphi : P(\ulcorner\varphi\urcorner) = 0 \}$ una teoria, con $P$ una funzione ricorsiva parziale,
    dimostrare che esiste una teoria $T'$ equivalente a $T$ e ricorsivamente assiomatizzabile.
\end{exercise}
...
\begin{proof}
    ...
    \[ g(z) := \begin{cases}
        ...
        1 & \text{altrimenti}
    \end{cases} \]
    infatti $g(z)=0$ se e solo se $z$ è il codice di un assioma di $T'$, per cui $T'$ è ricorsivamente assiomatizzabile.
\end{proof}
...
\begin{proposition}[Completamento decidibile]
    Sia $T$ una teoria coerente e decidibile, i.e. $T = \{\sigma | P(\ulcorner\sigma\urcorner) = 0\}$ con $P$ una funzione ricorsiva totale o $\{\ulcorner \varphi \urcorner | T \vdash \varphi\}$ è ricorsivamente enumerabile,
    allora esiste un'estensione coerente $T'$ di $T$ che è completa e decidibile.
\end{proposition}
...
\begin{proof}
    ...
    \[ T_0:=T \]
    \[ T_{i+1}:=\begin{cases}
        T_i\cup\{\varphi_i\} & \text{se $i = \ulcorner \varphi_i \urcorner$ e $T_i\nvdash\neg\varphi_i$}\\
        T_i & \text{altrimenti}
    \end{cases} \]
    cioè in ogni step se una formula è già conseguenza della teoria aggiungiamo la sua negazione, altrimenti non faccio nulla, e alla fine poniamo $T' := \bigcup_{i\in\NN} T_i$.
    Per essere precisi nella costruzione 
\end{proof}
...
\begin{proof}
    Se $\mathsf Q$ fosse decidibile, allora per la proposizione precedente esisterebbe un'estensione coerente e completa di $\mathsf Q$, che è impossibile per il teorema di Rosser.
    Idem per $\mathsf{PA}$, che estende $\mathsf Q$.
\end{proof}
...
\begin{proof}
    ...
    ma allora la decidibilità di $T$ implica la decidibilità di $\mathsf{Q}$ in quanto basta testare se $T$ dimostra $\mathsf{Q}1 \land \ldots \land \mathsf{Q}7 \to \varphi$ per ogni formula $\varphi$, controesempio al teorema di Rosser.
\end{proof}
