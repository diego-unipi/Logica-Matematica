...
\begin{proof}
    ...
    \begin{itemize}
        \item[$\boxed{\land,\lor}$] Portando entrambe in forma premessa, posso accodare i blocchi esistenziali esterni e lasciare dentro una congiunzione/disgiunzione di formule di classe $\Pi_{n-1}^0$; resto quindi in $\Sigma_n^0$.
        \item[$\boxed{\exists}$] È banale: aggiungere un blocco esistenziale esterno non aumenta la complessità oltre $\Sigma_n^0$.
        \item[$\boxed{\forall y \le x}$] Scrivo $\varphi(x,y)$ in forma $\exists z_1\ldots\exists z_r\,\theta(x,y,\mathbf z)$ con $\theta\in\Pi_{n-1}^0$. Allora
