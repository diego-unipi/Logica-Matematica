...
\begin{theorem}[Secondo teorema di incompletezza di Gödel]
    $\mathsf{PA} \nvdash \neg\square\bot$, ovvero $\mathsf{PA}$ non dimostra la propria incoerenza.
\end{theorem}
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        dove $\mathsf{PA} \vdash \square(\bot \to \gamma)$ perché $\mathsf{PA} \vdash \bot \to \gamma$ (è El$_\bot$) unito con la (1); mentre $\mathsf{PA} \vdash (\square \bot \land \square(\bot \to \gamma)) \to \square\gamma$ è la (2) del lemma.
        \item \textcolor{purple}{$\mathsf{PA} \vdash \neg\square\bot \to \gamma$}, come prima usiamo $\neg\square \gamma$ al posto di $\gamma$, e dimostriamo $\mathsf{PA} \vdash \neg\square\bot \to \neg\square\gamma$ usando la contronominale. [COMPLETARE]
    \end{itemize}
\end{proof}

SISTEMARE

\begin{remark}[Osservazioni filosofiche sul secondo teorema di incompletezza]
    ...
    Non è detto, a priori, che se dovessimo scrivere una formula che dice ``Peano è coerente'' finiremmo sulla formula $\neg\square\bot$.\\
    Ad esempio, se definissimo Peano in questo modo, enumerando tutti gli assiomi e aggiungendo quelli di base ed uno alla volta quelli dello schema di induzione,
    fermandomi non appena arrivo al primo assioma che dà incoerenza; questa è una definizione del tutto corretta di Peano, perché sappiamo che di fatto non beccheremo mai 
    un assioma che dà incoerenza, quindi gli assiomi ci sono tutti, e questa teoria sarà in grado di dimostrare la propria coerenza, perché è definita in modo che sia così - basterebbe usare compattezza.\\
    Quindi il fatto che Peano non sia in grado di dimostrare la propria coerenza è un fatto che dipende da come è descritta Peano e la sua coerenza; se trovassimo un'altra descrizione di Peano e della sua coerenza magari riusciremmo a dimostrarla.
    Quello che alla fine abbiamo dimostrato noi è che, con le codifiche che abbiamo scelto, non riusciamo a dimostrare una certa formula, all'interno della quale leggo che Peano non può dimostrare la propria coerenza.\\
    $\mathsf{ZF}$, che è più potente, non è in grado di dimostrare la propria coerenza, ma è in grado di dimostrare vari teoremi che caratterizzano la coerenza; ad esempio, se ci fosse un cardinale inaccessibile, allora $\mathsf{ZF}$ sarebbe coerente - e quindi $ZF$ non dimostra che c'è un cardinale inaccessibile;
    questa implicazione è proprio possibile grazie al fatto che ZF è abbastanza potente da poter connettere la formula che esprime la coerenza di ZF con l'esistenza di un cardinale inaccessibile; in generale non è detto che la teoria stessa sia in grado in generale di rendersi conto dei diversi modi di dire "$T$ è coerente".
\end{remark}