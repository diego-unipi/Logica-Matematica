...
\begin{example}[Successione di Fibonacci PR - v.2]
    ...
    cioè la funzione tale che $h(\alpha) = g(\alpha, \cdot)$. Sia $\alpha$ un punto fisso di $h$,
    per cui $u_1(\alpha, x) = u_1(h(\alpha), x)$, e dico che $f(x) := u_1(\alpha, x)$, per cui la successione di Fibonacci coincide con una funzione ricorsiva.
    ...
    \end{align*}
    e, per la definizione di $g$, si ottiene che $u_1(\alpha, x)$ soddisfa esattamente la definizione ricorsiva di $f$. Se dimostro nella metateoria che
...
\begin{definition}[Insiemi decidibili, semidecidibili e ricorsivamente enumerabili]
    ...
    \begin{itemize}
        ...
        \item \vocab{ricorsivamente enumerabile} se esiste $f : \NN \to \NN^k$ ricorsiva totale tale che $R = \{f(n) | n \in \NN\}$, i.e. se $R$ è l'immagine di una funzione ricorsiva totale.
    \end{itemize}
\end{definition}
