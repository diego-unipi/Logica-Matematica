...
\begin{proof}
    ...
    dove il $\dotminus 1$ serve perché $\blacktriangleright$ è codificato da 2, e $U$ è PR per composizione di funzioni PR.
\end{proof}
...
\begin{soln}
    ...
    la funzione PR $\textsf{Conj}(a,b) := \ulcorner\varphi_a \land \varphi_b\urcorner$, di definire $H(1,k) := k, H(n+1,k) := \textsf{Conj}(H(n,k),k)$ e di definire $h(n) := H(n,f(n))$, allora otterremo addirittura che $h$ è PR.}).
    ...
    \[ g(z) := \begin{cases}
        ...
        1 & \text{altrimenti}
    \end{cases} \]
    che è totale, anzi PR (definizione per casi su insieme totale - motivo per cui limitiamo la ricerca ai primi $z$) e vale che $T' = \{z : g(z) = 0\}$.\\
    \textcolor{MidnightBlue}{Moralmente abbiamo codificato nell'assioma stesso l'$x$ che codice quell'assioma, ripetendolo l'AND $x$ volte.}
\end{soln}
\pagebreak
\begin{notation}
    D'ora in poi, intenderemo equivalentemente le espressioni ricorsivamente assiomatizzabile e decidibilmente assiomatizzabile. E di fatto 
    col teorema di Craig abbiamo addirittura che ogni teoria ricorsivamente assiomatizzabile è equivalente ad una teoria con insieme di assiomi PR,
    cosa che servirà più avanti.
\end{notation}
...
\begin{proof}
    ...
    $$ g(n, 0) := \begin{cases} \langle \#\forall, 8, n \rangle & \text{se } \mathrm{free}(n, 0) = 0 \\ n & \text{altrimenti} \end{cases} $$
    $$ g(n, k+1) := \begin{cases} \langle \#\forall, (k+1)+8, g(n, k) \rangle & \text{se } \mathrm{free}(n, k+1) = 0 \\ g(n, k) & \text{altrimenti} \end{cases} $$
