...
\begin{soln}
    ...
    che è ricorsiva totale - per il teorema s-m-n - e se $(\alpha,x) \in H_1^\complement$, allora $u_1(\alpha,x) = \bot$, per cui $g_{\alpha,x} \equiv \bot$, e quindi il suo codice sta in $TOT^\complement$;
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item \textcolor{MidnightBlue}{$n_1 \implies n_1 + 1$} Assumiamo che $n_1 + 1 < n_2$, come ipotesi induttiva abbiamo che $\forall m \in \NN \; n_1 < m \implies \mathsf{Q} \vdash \neg \, \ol{n_1} = \ol m$, e vogliamo dimostrare che $\mathsf{Q} \vdash \neg \, \ol{n_1 + 1} = \ol{n_2}$.
...
\begin{lemma}[Rappresentazione di funzioni ricorsive totali in $\mathsf{Q}$]
    Sia $f : \NN^k \to \NN$ ricorsiva \purple{totale}, allora c'è $\varphi(x_1,\ldots,x_k,y)$ di classe $\Sigma_1^0$ tale che:
