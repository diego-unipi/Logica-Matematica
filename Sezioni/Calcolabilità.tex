...
\begin{proof}
    Vediamo le implicazioni in ordine circolare, fare attenzione al caso in cui $R = \varnothing$.
    \begin{itemize}
...
\textcolor{MidnightBlue}{L'idea è, essendo $H_1$ semidecidibile, allora, per quanto visto, è di classe $\Sigma_1^0$, e quindi esiste una formula $\varphi(x,y) \in \Sigma_1^0$
tale che $H_1 = \{N \models \varphi(x,y)\}$. A questo punto considerando la codifica della formula ottenuta sostituendo $(\alpha,x)$ alle due variabili libere di $\varphi$, otteniamo
un numero naturale, e se l'insieme sopra fosse decidibile, allora stabilire se $\NN$ dimostra o meno quest'affermazione, e di conseguenza potremmo decidere l'halting set$\lightning$.}
...
Dal lemma, la cui dimostrazione è conseguenza del fatto generale visto sulle formule $\Delta_0^0$ e $\Sigma_1^0$ in precedenza, segue in particolare che se $M \models \mathsf{Q}$ e $a_1,\ldots,a_k \in \NN$, allora $M \models \psi(\ol{a_1},\ldots,\ol{a_k})$ se e solo se $\mathsf{Q} \vdash \psi(\ol{a_1},\ldots,\ol{a_k})$.
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            questo per l'equivalenza data dal lemma iniziale dice però che $f(x_1, \dots, x_k) = y$, e quindi non può esistere un tale modello con un tale $y$.
        ...
        deve valere necessariamente che $M \models t = \ol{f(x_1, \dots, x_k)}$. Però, le disuguaglianze all'inizio, essendo $\ol N$ numerale, implicano per un lemma,
        che $z_1, \dots, z_l, t$ sono standard. Quindi, eliminando gli AND si ha:
        \[ M \models \psi(\ol{x_1}, \dots, \ol{x_k}, \ol t, \ol{z_1}, \dots, \ol{z_l}) \]
        formula $\Delta_0^0$, per cui, per assolutezza si ottiene $\mathsf{Q} \vdash \psi(\ol{x_1}, \dots, \ol{x_k}, \ol{f(x_1, \dots, x_k)})$.
    \end{itemize}
\end{proof}
...
\begin{proof}
    ...
    con $\textsf{Neg}(x) = \langle \#\neg, x\rangle$ la funzione PR che associa ad una formula il codice della sua negazione. Usiamo il lemma di diagonalizzazione di Gödel su questa formula e troviamo $\gamma$ tale che:
...
\begin{remark}[Semidecidibile $\implies$ ricorsivamente assiomatizzabile]
    ...
    $$\begin{tikzcd}
        & {\text{ricorsivamente assiomatizzabile}} \\
        {\text{semidecidibile}} & {} \\
        & {\text{decidibilmente assiomatizzabile}}
        \arrow[Leftrightarrow, from=1-2, to=3-2]
        \arrow[Rightarrow, from=2-1, to=1-2]
        \arrow[Rightarrow, from=3-2, to=2-1]
\end{tikzcd}$$
\end{remark}
...
\begin{proof}
    ...
    e $\mathsf{Cl}(n) := g(n, \length(n))$ (poiché $\cons(x,y) \geq \max{\{x,y\}}$ non posso avere codici in cui la codifica di una variabile superi $n$). Fatto questo $B$ è semidecidibile in quanto la formula che lo definisce è $\Sigma_1^0$.
\end{proof}
