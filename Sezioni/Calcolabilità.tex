...
\begin{exercise}[Teorema di Craig]
    Sia $T := \{\varphi : P(\ulcorner\varphi\urcorner) \ne \bot\}$ ricorsivamente assiomatizzabile, allora esiste una teoria decidibilmente assiomatizzabile, tale che $T$ e $T'$ sono equivalenti.
\end{exercise}
...
\begin{soln}
    ...
    \[ g(z) := \begin{cases}
        ...
        1 & \text{altrimenti}
    \end{cases} \]
    che è totale (definizione per casi su insieme totale - motivo per cui limitiamo la ricerca ai primi $z$) e vale che $T' = \{z : g(z) = 0\}$.
\end{soln}
...
\begin{proof}
    ...
    ``$\nth(d,i)$ è un assioma'' è esprimibile come predicato PR, perché per ipotesi $T$ è ricorsivamente assiomatizzabile, quindi esiste una funzione ricorsiva totale da mettere in quel posto.\footnote{In verità questo ci dà una funzione ricorsiva totale, non necessariamente PR, ma per l'osservazione sulla notazione sotto, non serve davvero che sia PR, basta che sia ricorsiva totale.
    Tuttavia si può aggiustare rendendolo PR in vari modi, ma non lo farò.}
...
\begin{remark}
    La formula $\textsf{free}(y,i)$, che dice che $x_i$ è libera in $y$, 
\end{remark}
    ...
    \begin{itemize}
        ...
        \begin{align*}
            T \vdash \gamma &\iff \NN \models \exists d \, \mathsf{Dim}_T(d,\ulcorner\gamma\urcorner) &&(\text{lemma}) \\
                            &\iff \mathsf{Q} \vdash \exists d \, \mathsf{Dim}_T(d,\ulcorner\gamma\urcorner) &&(\Sigma_1^0) \\
                            &\iff \mathsf{Q} \vdash \neg \,\gamma && (\text{equivalenza di $\gamma$}) \\
                            &\implies T \vdash \neg \,\gamma \; \lightning && (T \vdash \mathsf{Q})
        \end{align*} 
        \item Assumiamo $T \vdash \neg \,\gamma$, allora:
        \begin{align*}
            T \vdash \neg \,\gamma &\iff T \vdash \exists d \, \mathsf{Dim}_T(d,\ulcorner\gamma\urcorner) && (\text{equivalenza di $\gamma$}) \\
...
\begin{remark}[Ricorsivamente assiomatizzabile $\implies$ semidecidibile]
    Se $T$ è semidecidibile, allora il predicato di dimostrabilità $\mathsf{Dim}_T$ è ricorsivo totale per il lemma. Per cui posso definire i codici delle conseguenze di $T$ come:
    \[ \{\ulcorner\sigma\urcorner : T \vdash \sigma\} = \{\ulcorner\sigma\urcorner : \exists d \, \mathsf{Dim}_T(d,\ulcorner\sigma\urcorner)\} \]
    dove sto considerando la formula, di classe $\Sigma_1^0$, associata al predicato di dimostrazione, e quindi $T$ è decidibilmente assiomatizzabile.
...
\begin{remark}[Semidecidibile $\implies$ ricorsivamente assiomatizzabile]
    Se $T$ è semidecidibile, allora imitando la dimostrazione del teorema di Craig, possiamo costruire una teoria $T'$ decidibilmente assiomatizzabile, tale che $T$ e $T'$ sono equivalenti, e quindi $T$ è ricorsivamente assiomatizzabile.
