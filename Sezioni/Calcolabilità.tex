...
\begin{example}[Funzione modulo]
    La funzione $x \bmod y = z \overset{\text{def}}{\iff} z < y \land y \mid x - z \land y \ne 0$, cioè il rappresentante di $x$ modulo $y$, è una funzione PR poiché è definita per ricorsione e per casi:
...
\begin{exercise}[Difficile]
    ...
    in modo che la TM non possa usare come informazione il numero di passi fatti, e quindi deve attraversare il confine almeno $l/2$ volte, e ogni volta deve fare almeno $l/2$ passi, per cui ottengo $\Omega(l^2)$.}
\end{exercise}
...
\begin{soln}
    ...
    \end{tabular}
    \end{table}
    \textcolor{MidnightBlue}{Per essere precisi, dopo la simulazione di $F$ bisognerebbe tornare a sinistra fino al $\blacktriangleright$ interno e riscriverlo come $0$, così da ripristinare il separatore e mantenere il formato unario dell'input.}
\end{soln}
...
\begin{soln}
    ...
    Per $\length$ basterà osservare che $\boxed{\length(l) = \#\{i < l| \cdr^i(l) \ne 0\}}$, cioè quante volte si può fare $\cdr$ prima di arrivare ad avere seconda componente 0 in $\cons$ - $\cons$ non può fare 0;
    ...
    Infine si osserva che $l$ è un bound sufficiente sotto cui controllare perché una lista codificata con $l$ non può avere più di $l$ elementi (per le proprietà di $\cons$).\\
...
\begin{soln}
    ...
    \[ g(l,s):=\begin{cases}
        ...
        \cat(\underbrace{\car(s)}_{\mathclap{\langle a_0,\ldots, a_{\length(s)\dotminus 2} \rangle}},\cons(\nth(l,\length(s)\dotminus 1),0)) & \text{altrimenti}
    \end{cases} \]
...
\end{soln}
\pagebreak
\begin{remark}[Non serve davvero la ricorsione sul decorso dei valori per $\head$]
    In realtà, si può dimostrare che $\head$ è PR anche senza usare il lemma di ricorsione sul decorso dei valori, infatti basta dare la definizione ricorsiva:
    \[ \head(l,0) := 0, \head(l,k+1) := \begin{cases}
        \cat(\head(l,k), \cons(\nth(l,k),0)) & \text{se $k < \length(l)$} \\
        \head(l,k) & \text{se $k \geq \length(l)$}
    \end{cases} \]
\end{remark}
...
\begin{proof}
    ...
    \begin{proof}
        ...
        \[ \text{nastro} = \langle \underbrace{a_0,\ldots,a_{p-1}}_{=:\textcolor{MidnightBlue}{\alpha}}, \underbrace{a_p}_{=:\textcolor{MidnightBlue}{\sigma}}, \underbrace{a_{p+1},\ldots}_{=:\textcolor{MidnightBlue}{\beta}} \rangle \]
        dove:
        \begin{itemize}
            \item $\textcolor{MidnightBlue}{\alpha} := \head(\text{nastro},\text{posizione}) = \langle a_0,\ldots,a_{p-1}\rangle = \head(\nth(c,2),\nth(c,1))$,
            \item $\textcolor{MidnightBlue}{\sigma} := \nth(\text{nastro},\text{posizione}) = a_p = \nth(\nth(c,2),\nth(c,1))$,
            \item $\textcolor{MidnightBlue}{\beta} := \cdr^{\text{posizione}+1}(\text{nastro}) = \langle a_{p+1},\ldots\rangle = \cdr^{\nth(c,1)+1}(\nth(c,2))$.
        \end{itemize}
    ...
    a questo punto prendo il nastro della configurazione finale e ci applico una funzione che conta gli 1, che darà proprio l'output di $f$ - a meno di sottrarre 1 perché abbiamo detto all'inizio che $\blacktriangleright$ corrisponde a 2, posso definire:
...
\begin{corollary}[Forma normale - di Kleene - per le funzioni ricorsive]
...
\begin{proof}
    ...
    dove il $\dotminus 1$ serve perché $\blacktriangleright \ne 0$ - e per $\texttt{conta\_uni}$ vale 1, e $U$ è PR per composizione di funzioni PR.
\end{proof}

Per cui, d'ora in avanti, con forma normale per le funzioni ricorsive, ci riferiremo al test $T_k$ che verifica se la funzione ricorsiva termina su un dato input, ed alla $U$ che ne estrae il testimone.
...
\begin{proof}
    ...
    \[ 1 \to \underbrace{\exec_{n+1,1} s \to \exec_{n+1,1} s \to \ldots \to \exec_{n+1,1} s}_{\text{$x_1$ volte}} \to 2 \]
...
\begin{soln}
    ...
    che è ricorsiva totale.\footnote{Notare che provare a scegliere $\texttt{k}\circ f(n) := \texttt{k} \circ f(n)$ non funziona perché la funzione ottenuta non è necessariamente totale.}
\end{soln}
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \[ f(\mathbf x) \ne \bot \iff \exists n \;T_k(\ulcorner f \urcorner, \mathbf x, n) = 0 \]
        e, in tal caso, $f(\mathbf x) = U(\mu_n(T_k(\ulcorner f \urcorner, \mathbf x, n))))$. Non possiamo usare direttamente $\mathbf x \mapsto U(\mu_n(T_k(\ulcorner f \urcorner, \mathbf x, n)))$, in quanto $U$ prende in input il valore della minimizzazione che dipende da $\mathbf x$, e può essere $\bot$, quindi $U$ potrebbe non essere definita per alcuni valori di $\mathbf x$.\\
        Possiamo aggirare questo problema considerando la lista $\langle \mathbf x, n \rangle$, e definendo una funzione che controlla se la minimizzazione di $T_k$ è ben definita per questi valori, nel cui caso restituisce $\mathbf x$, altrimenti restituisce un elemento qualsiasi $\mathbf c \in R$:
        \[ g(n) := \begin{cases}
           ``\mathbf x\text{"} &\text{se $T_k(\ulcorner f \urcorner, \nth(n,0), \ldots, \nth(n,k-1),\purple{\nth(n,k)}) = 0$} \\
            \mathbf c & \text{altrimenti}
        \end{cases} \]
        dove ``$\mathbf x$'' è in realtà $(\nth(n,0), \ldots, \nth(n,k-1))$, per cui $g$ è PR. Inoltre $R = \Img g$, infatti: se $\mathbf x \in \Img g$,
        allora esiste $n$ tale che $g(n) = \mathbf x$, per cui $T_k(\ulcorner f \urcorner, \mathbf x, \nth(n,k)) = 0$, e quindi $f(\mathbf x) \ne \bot$;
        viceversa, se $\mathbf x \in R$, allora $f(\mathbf x) \ne \bot$, per cui esiste $n$ tale che $T_k(\ulcorner f \urcorner, \mathbf x, n) = 0$, e quindi prendendo $m := \langle \mathbf x, n \rangle$ si ha $g(m) = \mathbf x$.\\
         Se $R = \varnothing$, allora è ricorsivamente enumerabile per definizione.
        ...
        \[ h(\mathbf y) := 0_1(\mu_n(g(\mathbf y, n))) \]
        che è ricorsiva parziale, e vale che $h(\mathbf x) = 0 \ne \bot$ se e solo se $\exists n \; g(\mathbf x,n) = 0$, ovvero se e solo se esiste $n$ tale che $f(n) = \mathbf x$.
    \end{itemize}
\end{proof}
...
\begin{proof}
    ...
    dove $\chi_{=}$ è il predicato PR che definisce l'uguaglianza, e la funzione è totale in quanto ogni $\mathbf x$ sta in $R$ o $R^\complement$, per cui una tra $f$ e $g$ becca necessariamente $\mathbf x$.
    Quindi la funzione definita restituisce il minimo $n$, modulo 2, tale che $h(n) = \mathbf x$, pertanto tale funzione farà 0 sui valori di $R$ e 1 sui valori di $R^\complement$.\\
...
\begin{remark}[Definizione per casi decidibili di funzioni ricorsive]
    ...
    \end{cases} \]
    con $g$ e $h$ funzioni ricorsive totali, è ricorsiva parziale in quanto:
...
\begin{soln}
    ...
    \[ f(x_1,\ldots, x_n) := \begin{cases}
        u_n(x_1, x_1, \ldots, x_1) + 1 & \text{se $(x_1, x_1, \ldots, x_1) \in H_n$} \\
        0 & \text{altrimenti}
    \end{cases} \]
...
\begin{soln}
    ...
    che è ricorsiva totale - per il teorema $s-m-n$ per passare al codice si compone con $s_0^1$ che è PR - e se $(\alpha,x) \in H_1^\complement$, allora $u_1(\alpha,x) = \bot$, per cui $g_{\alpha,x} \equiv \bot$, e quindi il suo codice sta in $TOT^\complement$;
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item[$\boxed{\forall y \le x}$] Scriviamo $\varphi$ in forma premessa $\exists z_1\ldots\exists z_r\,\theta$ con $\theta\in\Pi_{n-1}^0$. Allora:
        \[ \forall y\le x\,\varphi \leftrightarrow \exists b\,\underbrace{\forall y\le x\,\exists z_1\le b\ldots\exists z_r\le b}_{\text{quantificatori limitati}}\,\theta \]
...
\begin{remark}[Corrispettivo insiemistico della chiusura delle classi aritmetiche]
    ...
    \begin{itemize}
        ...
        \item se $R \subseteq \NN^{k+1}$ di classe $\Sigma_n^0$, definito dalla formula $R(\mathbf x,y)$, allora $\forall y \leq z \; R(\mathbf x,y)$ corrisponde all'insieme:
        \[ S := \{(\mathbf x,\purple{z}) \in \NN^{k+1} : \forall y \leq z \; R(\mathbf x,y)\} = \{(\mathbf x,z) : \mathbf x \in \bigcap_{y \leq z} R_y\} \]
        dove $R_y := \{\mathbf x \in \NN^k : R(\mathbf x,y)\}$. Analogamente $\exists y \leq z \; R(\mathbf x,y)$:
        \[ S = \{(\mathbf x,\purple{z}) \in \NN^{k+1} : \exists y \leq z \; R(\mathbf x,y)\} = \{(\mathbf x,z) : \mathbf x \in \bigcup_{y \leq z} R_y\} \]
   \end{itemize}%Manca esistenziale
\end{remark}
...
\begin{lemma}[Per ogni funzione ricorsiva c'è una formula equivalente di classe $\Sigma_1^0$]
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \begin{remark}[Complessità di $\beta(a,b,k)$]
            ...
            \[ z < b(k+1)+1 \land \exists t (z + t(b(k+1)+1) = a) \]
        ...
        o meglio:
        \begin{align*}
            \varphi_f(\mathbf x,y,z) := \exists a \exists b \; &\varphi_g(\mathbf x,\beta(a,b,0)) \\
                                                             &\land \forall i < y \, \varphi_h(\mathbf x,i,\beta(a,b,i),\beta(a,b,i+1)) \\
                                                             &\land \beta(a,b,y) = z
        \end{align*}
        questa formula è quasi corretta e di classe $\Sigma_1^0$; la cosa mancante è che $\beta$ non è un simbolo di funzione, ma una abbreviazione per la formula sopra, quindi non possiamo fare la composizione davvero, quello che andrebbe fatto
        è riscrivere la formula nel secondo passaggio come:
        \[ \forall i < y \, \exists m \exists n \; (\beta(a,b,i) = n) \land (\beta(a,b,i+1) = m) \land \varphi_h(\mathbf x,i,n,m) \]
        e quella del primo come:
        \[ \exists w \; (\beta(a,b,0) = w) \land \varphi_g(\mathbf x,w) \]
    \end{itemize}
\end{proof}
...
\begin{proof}
    Procediamo per induzione strutturale sulla costruzione di una formula di classe $\Delta_0^0$, da cui si considera l'insieme definito e si dimostra che è un predicato PR.
    \begin{itemize}
        ...
        \item[$\boxed{\forall y \le x}$] Se $\varphi = \forall y \leq z \; \psi(\mathbf x,y)$, allora, detto $R := \{\NN \models \psi(\mathbf x,y)\} \subseteq \NN^{k+1}$, per ipotesi induttiva si ha che esiste una funzione $T : \NN^{k+1} \to \NN$ PR tale che:
        \[ R = \{T(\mathbf x,y) = 0\} \]
        Poiché:
        \[ \{\NN \models \forall y \leq z \; \psi(\mathbf x,y)\} = \{(\mathbf x,z) \in \NN^{k+1} : \forall y \leq z \; (\mathbf x,y) \in R\} \]
        ovvero è sufficiente controllare che per tutti gli $i \leq z$ vale $T(\mathbf x,i) = 0$, e questo è precisamente il luogo di zero del predicato PR che conta gli 
        elementi, sotto un certo valore di un altro predicato:
        \[ \{ \#\{i \leq z : T(\mathbf x,i) \ne 0\} = 0 \} = \{(\mathbf x,z) \in \NN^{k+1} : \forall i \leq z \; T(\mathbf x,i) = 0\} \]
    \end{itemize}
    Il caso di $\exists y \leq z$ è duale a quello di $\forall y \leq z$, in quanto $\exists y \leq z \; \psi(\mathbf x,y)$ è equivalente a $\neg\, \forall y \leq z \; \neg\, \psi(\mathbf x,y)$, e quindi segue dai casi precedenti.
\end{proof}

E quindi possiamo dimostrare la freccia $\impliedby$ del teorema.

\begin{proof}
    Sia $S \in \Sigma_1^0$, allora $S$ è della forma:
    \[ S = \{\mathbf x \in \NN^k : \NN \models \exists y_1\ldots \exists y_h \varphi(\mathbf x,y_1,\ldots,y_h)\}\]
    con $\varphi$ di classe $\Delta_0^0$. Consideriamo l'insieme $\{\NN \models \varphi(\mathbf x,y_1,\ldots,y_h)\} \subseteq \NN^{k+h}$, per il lemma precedente esiste un predicato PR $T : \NN^{k+h} \to \NN$
    tale che $\{\NN \models \varphi(\mathbf x,y_1,\ldots,y_h)\} = \{T(\mathbf x,y_1,\ldots,y_h) = 0\}$. Possiamo quindi riscrivere $S$ come:
    \[ S = \{\mathbf x \in \NN^k : \exists y_1\ldots \exists y_h \; (\mathbf x, y_1,\ldots,y_h) \in T\} \]
    a questo punto per decidere se un $\mathbf x$ appartiene a $S$ o meno è sufficiente stabilire se esistono $y_1,\ldots,y_h$ tali che $T(\mathbf x,y_1,\ldots,y_h) = 0$, e possiamo fare questo lavoro usando la minimizzazione,
    ovvero chiedendo che $\mu_{\mathbf y}(T(\mathbf x,y_1,\ldots,y_h)) \in \NN$. Per cui la funzione ricorsiva di cui $S$ è luogo di zero è semplicemente mandare a 0 il controllo che un elemento $\mathbf x$ appartenga a $T$, ovvero:
    \[ S = \{0_1 \circ \mu_{\mathbf y}(T(\mathbf x,y_1,\ldots,y_h)) = 0\} \]
    dove per minimizzazione su un vettore intendiamo la minimizzazione sulla codifica $\langle \mathbf y \rangle = n$, e quindi nella formula ci vanno $\nth(n,i)$ al posto di $y_i$.
\end{proof}
...
\begin{corollary}[Teorema di indecidibilità di Church]
    L'insieme $\{ \ulcorner \varphi \urcorner : \NN \models \varphi\}$ non è semidecidibile.
\end{corollary}
...
Diamo per buona l'idea intuitiva di numerazione di Gödel come una codifica di formule, dimostrazioni, eccetera in numeri naturali, e che sia compatibile con le operazioni semantiche.

\begin{theorem}[Rosser]
    Sia $T$ una $L$-teoria tale che:
    \begin{enumerate}[(i)]
        \item $T \vdash \mathsf{Q}$, ovvero $T$ estende $\mathsf{Q}$,
        \item $T$ è ricorsivamente assiomatizzabile - qualsiasi cosa significhi per ora,
        \item $T$ è coerente,
    \end{enumerate}
...
\begin{proposition}[Equivalenza tra validità in $\NN$ e dimostrabilità in $\mathsf{Q}$]
    Sia $\varphi$ una formula aritmetica $\purple{\text{chiusa}}$, valgono i seguenti fatti.
    \begin{itemize}
...
\begin{proof}
    ...
    \begin{itemize}
        \item \textcolor{MidnightBlue}{$t = 0$} In questo caso $n = 0$, indipendentemente dalla valutazione essendo chiuso, e $\ol 0 = 0$ per definizione di numerale, quindi $Q \vdash t = \ol n$, per Wk + In$_{=}$.
        \item \textcolor{MidnightBlue}{$t = s(t')$} Per ipotesi induttiva, detto $m := \{\ldots\}_\NN t'$, vale che $Q \vdash t' = \ol m$, e quindi - modus ponens - $Q \vdash s(t') = s(\ol m)$, inoltre per definizione di numerale $s(\ol m) = \ol{m+1}$, e per definizione di interpretazione $\{\ldots\}_\NN s(t') = m + 1$.
        ...
        \[\forall a,b,c \in \NN \quad a+b = c \implies Q \vdash \ol a + \ol b = \ol c \]
        ...
        Assumiamo che $a + b = c \implies Q \vdash \ol a + \ol b = \ol c$, e vediamo che $a + s(b) = c$, allora $\mathsf{Q} \vdash \ol a + s(\ol{b}) = s(\ol{c})$, e per $\mathsf{Q}4$ vale che $\mathsf{Q} \vdash \ol a + s(\ol{b}) = s(\ol a + \ol b)$, 
        che, per El$_{=}$, è equivalente a $\mathsf{Q} \vdash \ol a + s(\ol{b}) = s(\ol c)$, e per definizione di numerale $s(\ol c) = \ol{c+1}$, che corrisponde alla tesi perché semanticamente $a + s(b) = s(a+b) = s(c)$.
        \item \textcolor{MidnightBlue}{$t = t_1 \cdot t_2$} Analogo al caso precedente.
    \end{itemize}
\end{proof}

\begin{lemma}[Caso base del teorema di equivalenza]
    Sia $\varphi$ una $L_{\text{arit}}$-formula atomica chiusa, allora:
    \[ \NN \models \varphi \iff \mathsf{Q} \vdash \varphi \quad \text{e} \quad \NN \models \neg\, \varphi \iff \mathsf{Q} \vdash \neg\, \varphi \]
\end{lemma}

\begin{proof}
    Osserviamo in primis che ci basta solo dimostrare l'implicazione $\NN \models \varphi \implies \mathsf{Q} \vdash \varphi$, poiché infatti $\mathsf{Q} \vdash \psi$ se e solo se $\mathsf{Q} \models \psi$ per completezza,
    e poiché $\NN$ è un modello di $\mathsf{Q}$, allora $\NN \models \psi$.\\
    Assumiamo che $\NN \models \varphi$ e vediamo che $\mathsf{Q} \vdash \varphi$. Le uniche $L_{\text{arit}}$-formule atomiche chiuse, non banali, sono uguaglianze di termini chiusi $t_1 = t_2$, procediamo con la prima implicazione:
    \begin{align*}
        \NN \models t_1 = t_2 &\iff \{\ldots\}_\NN t_1 = \{\ldots\}_\NN t_2 \\
                            &\implies \mathsf{Q} \vdash \ol{\{\ldots\}_\NN t_1} = \ol{\{\ldots\}_\NN t_2}
    \end{align*}
    per il lemma $\mathsf{Q} \vdash t_1 = \ol{\{\ldots\}_\NN t_1}$ e $\mathsf{Q} \vdash t_2 = \ol{\{\ldots\}_\NN t_2}$, usando due volte El$_{=}$ si conclude.\\
    Per l'altra implicazione, assumiamo che $\NN \models \neg\, (t_1 = t_2)$, e supponiamo WLOG che $n_1 := \{\ldots\}_\NN t_1 < \{\ldots\}_\NN t_2 =: n_2$,
    allora per il lemma vale $\mathsf{Q} \vdash t_1 = \ol{n_1}$ e $\mathsf{Q} \vdash t_2 = \ol{n_2}$. Ci basta dimostrare quindi che $\mathsf{Q} \vdash \neg\, \ol{n_1} = \ol{n_2}$,
    per avere la tesi. Procediamo per induzione metateorica su $n_1$.
    \begin{itemize}
        \item \textcolor{MidnightBlue}{$n_1 = 0$} In questo caso $0 < n_2$ per ipotesi e vogliamo dimostrare che $\mathsf{Q} \vdash \neg\, \ol 0 = \ol{n_2}$. Dalle ipotesi segue che $\ol n_2$ è successore di qualche altro termine, e quindi
        il fatto che $\mathsf{Q} \vdash \neg\, \ol 0 = \ol{n_2}$ segue da $\mathsf{Q}1$ e $\mathsf{Q}7$.
        \item \textcolor{MidnightBlue}{$n_1 \implies n_1 + 1$} Assumiamo che $n_1 + 1 < n_2$, come ipotesi induttiva abbiamo che $\forall m < n_2 \; n_1 < m \implies \mathsf{Q} \vdash \neg\, \ol{n_1} = \ol m$, e vogliamo dimostrare che $\mathsf{Q} \vdash \neg\, \ol{n_1 + 1} = \ol{n_2}$.
        Poiché $n_1 + 1 < n_2$, allora $n_2 = s(l)$ con $n_1 < l$ per $\mathsf{Q}2$, quindi per ipotesi induttiva $\mathsf{Q} \vdash \neg\, \ol{n_1} = \ol l$, da cui:
        \[ \mathsf{Q} \vdash \neg\, s(\ol{n_1}) = s(\ol l) \implies \mathsf{Q} \vdash \neg\, \ol{n_1 + 1} = \ol{n_2} \]
    \end{itemize}
\end{proof}
...
\begin{lemma}[Ogni elemento non standard è maggiore di ogni elemento standard]
    ...
    \[ \forall n \in \NN \; M \models \ol n \leq \alpha \]
\end{lemma}
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        prendendo $x = \alpha$, e poiché vale $\mathsf{Q}3$, otteniamo la tesi.
        \item[$\boxed{n \implies n+1}$] Vogliamo dimostrare che $M \models \ol{n+1} \leq \alpha$, ovvero $M \models \exists y \; y + \ol{n+1} = \alpha$. Poiché $\alpha$ è non standard, per $\mathsf{Q}7$ vale $\alpha = s_M(\beta)$ con $\beta \in M$, e $\beta \leq \alpha$.
        Per ipotesi induttiva esiste $y$ tale che $y + \ol n = \beta$, e quindi per $\mathsf{Q}4$ si ottiene la tesi:
        \[ y + \ol{n+1} = s_M(y + \ol n) = s_M(\beta) = \alpha \]
    \end{itemize}
\end{proof}
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        Sia $\{t\}_\NN = n$, per un lemma sappiamo che $\mathsf{Q} \vdash t = \ol n$, quindi possiamo rimpiazzare $t$ con $\ol n$. Per definizione di quantificazione limitata la formula equivale a:
        \[ \forall x (x \leq \ol{n} \to \psi) \]
        e per un lemma visto sopra $\mathsf{Q} \vdash x \leq \ol n \leftrightarrow (x = \ol 0 \lor x = \ol 1 \lor \ldots \lor x = \ol n)$, per cui la formula equivale a:
        \[ \forall x (x = \ol 0 \to \psi) \land (x = \ol 1 \to \psi) \land \ldots \land (x = \ol n \to \psi) \]
        ...
        \[ \mathsf{Q} \vdash \psi[\ol 0/x] \land \psi[\ol 1/x] \land \ldots \land \psi[\ol n/x] \]
        ed a questo punto la tesi segue dall'ipotesi induttiva e dal caso dell'$\land$.
        \item \textcolor{MidnightBlue}{$\varphi = \exists x \leq t \; \psi$} Esattamente come nel caso di sopra si arriva a voler dimostrare:
        \[ \mathsf{Q} \vdash \psi[\ol 0/x] \lor \psi[\ol 1/x] \lor \ldots \lor \psi[\ol n/x] \]
        e anche in questo caso la tesi segue dal caso dell'$\lor$ e dall'ipotesi induttiva.
    \end{itemize}
    Notare che negli ultimi due casi non abbiamo verificato anche la tesi di (2), perché segue dalla tesi di (1) osservando che $\neg\, \forall x \leq t \; \psi$ è equivalente a $\exists x \leq t \; \neg\, \psi$, e viceversa.
\end{proof}
...
\begin{corollary}[Rappresentazione (del grafico) di funzioni ricorsive in $\mathsf{Q}$]
    ...
    \[ \forall x_1,\ldots,x_k\in\NN\;\forall y\in\NN \qquad f(x_1,\ldots,x_k)=y \iff \mathsf{Q} \vdash \varphi(\ol{x_1},\ldots,\ol{x_k},\ol y) \]
\end{corollary}

\begin{proof}
    Per il lemma sulla rappresentazione di funzioni ricorsive, mediante formule in $\Sigma_1^0$, sappiamo che esiste $\varphi \in \Sigma_1^0$ tale che:
    \[ \forall x_1,\ldots,x_k\in\NN\;\forall y\in\NN\; f(x_1,\ldots,x_k) = y \iff \NN \models \varphi(x_1,\ldots,x_k,y) \]
    Per il teorema di equivalenza tra validità in $\NN$ e dimostrabilità in $\mathsf{Q}$ per le formule \purple{chiuse} di classe $\Sigma_1^0$, sappiamo che:
    \[ \NN \models \varphi(x_1,\ldots,x_k,y) \iff \mathsf{Q} \vdash \varphi(\ol{x_1},\ldots,\ol{x_k},\ol y) \]
    mettendo assieme queste due cose si ottiene la tesi.
\end{proof}
...
\begin{lemma}[Rappresentazione di funzioni ricorsive totali in $\mathsf{Q}$]
    Sia $f : \NN^k \to \NN$ ricorsiva \purple{totale}, allora c'è $\varphi$ di classe $\Sigma_1^0$ tale che:
    \[ \forall x_1,\ldots,x_k\in\NN\; \mathsf{Q} \vdash \forall y \; (\varphi(\ol{x_1},\ldots,\ol{x_k},y) \leftrightarrow y = \ol{f(x_1,\ldots,x_k)}) \]
\end{lemma}

Serve un lemma, che non dimostriamo, ma si procede sempre per induzione strutturale sulle formule di classe $\Delta_0^0$.

\begin{lemma}[Sostituzione di numerali in formule di classe $\Delta_0^0$]
    Sia $\psi \in \Delta_0^0$ e siano $a_1,\ldots,a_k \in \NN$, allora, per ogni modello $M \models \mathsf{Q}$ vale:
    \[ M \models \psi(\ol{a_1},\ldots,\ol{a_k}) \iff \NN \models \psi(a_1,\ldots,a_k) \]
\end{lemma}

Dal lemma, la cui dimostrazione è conseguenza del fatto generale visto sulle formule $\Delta_0^0$ e $\Sigma_1^0$ in precedenza + il fatto che $\vdash$ e $\models$ sono equivalenti per le formule chiuse,
segue in particolare che se $M \models \mathsf{Q}$ e $a_1,\ldots,a_k \in \NN$, allora $M \models \psi(\ol{a_1},\ldots,\ol{a_k})$ se e solo se $\mathsf{Q} \vdash \psi(\ol{a_1},\ldots,\ol{a_k})$.

\begin{proof}
    Sia $\psi$ una formula di classe $\Delta_0^0$ tale che per ogni $x_1, \dots, x_k, y \in \NN$:
    \[ y = f(x_1, \dots, x_k) \iff \mathsf{Q} \vdash \underbrace{\exists z_1 \dots \exists z_l \, \psi(\ol{x_1}, \dots, \ol{x_k}, \ol{y}, z_1, \dots, z_l)}_{\in \Sigma_1^0} \]
    tale formula $\psi$ esiste per il lemma precedente. Consideriamo la formula $\varphi(x_1, \dots, x_k, y)$ definita come:
    \begin{align*}
        \exists N( & \exists z_1 \le N \dots \exists z_l \le N \, (y \le N \land \psi(x_1, \dots, x_k, y, z_1, \dots, z_l)) \\
                   & \land \forall z_1 \le N \dots \forall z_l \le N \, \forall t \le N \, (\psi(x_1, \dots, x_k, t, z_1, \dots, z_l) \to t = y) )
    \end{align*}
    che dice che esiste un limite $N$ tale che $y$ è minore o uguale a $N$, che ci sono testimoni $z_1, \dots, z_l$ minori o uguali a $N$ che soddisfano $\psi$; e che ogni testimone $t$ di $\psi$ minore o uguale a $N$ deve corrispondere a $y$.
    Notare che $\varphi$ è di classe $\Sigma_1^0$. Dimostriamo ora le due implicazioni della tesi, riscriviamo per comodità la formula $\varphi(x,y)$ come $\exists N \, \theta_1 \land \theta_2$.
    \begin{itemize}
        \item[$\boxed{\to}$] Fissiamo $x_1, \dots, x_k$ e mostriamo $\mathsf{Q} \vdash \forall y \, (\varphi(\ol{x_1}, \dots, \ol{x_k}, y) \to y = \ol{f(x_1, \dots, x_k)})$.\\
        Essendo la formula chiusa allora possiamo mostrare equivalentemente che vale $\mathsf{Q} \models \forall y \, (\varphi(\ol{x_1}, \dots, \ol{x_k}, y) \to y = \ol{f(x_1, \dots, x_k)})$.
        Procediamo per assurdo, allora esiste $M \models \mathsf{Q}$ e $y \in M$ tali che $M \models \varphi(\ol{x_1}, \dots, \ol{x_k}, y)$ e $M \models \neg\, (y = \ol{f(x_1, \dots, x_k)})$.\\
        Siccome $\varphi$ è $\purple{\exists N} \, \theta_1 \land \theta_2$, possiamo fissare $N \in M$ per cui valgano $\theta_1$ e $\theta_2$. Distinguiamo due casi a seconda che $N$ sia standard o non standard.
        \begin{itemize}
            \item \textit{Caso 1: $N$ è standard.} In questo caso, per la validità di $\theta_1$, ovvero:
            \[ M \models \exists z_1 \le N \dots \exists z_l \le N \, (y \le N \land \psi(\ol{x_1}, \dots, \ol{x_k}, y, z_1, \dots, z_l)) \]
            si ha che $y \in M$ è standard e che possiamo prendere $z_1, \dots, z_l \in M$ standard, per i quali quindi \purple{ci sono numerali associati}, e quindi, per il lemma si ottiene:
            \[ \NN \models \psi(\ol{x_1}, \dots, \ol{x_k}, \ol y, \ol z_1, \dots, \ol z_l) \]
            o meglio:
            \[ \mathsf{Q} \vdash \psi(\ol{x_1}, \dots, \ol{x_k}, \ol y, \ol z_1, \dots, \ol z_l) \]
            questo per l'equivalenza data dal lemma iniziale dice però che $\ol{f(x_1, \dots, x_k)} = y \in M$, e quindi non può esistere un tale modello con un tale $y$.
            \item \textit{Caso 2: $N$ non è standard.} In questo caso, consideriamo la formula $\theta_2$:
            \[ M \models \forall z_1 \le N \dots \forall z_l \le N \, \forall t \le N \, (\psi(\ol{x_1}, \dots, \ol{x_k}, t, z_1, \dots, z_l) \to t = y) \]
            dal lemma iniziale sappiamo che $y = f(x_1, \dots, x_k)$ è equivalente a $\mathsf{Q} \vdash \exists z_1 \dots \exists z_l \, \psi(\ol{x_1}, \dots, \ol{x_k}, \ol{f(x_1, \dots, x_k)}, z_1, \dots, z_l)$,
            a meno di passare in $\NN$ per assolutezza, sappiamo che esistono dei numerali $\ol{z_1}, \dots, \ol{z_l}$ tali che:
            \[ \mathsf{Q} \vdash \psi(\ol{x_1}, \dots, \ol{x_k}, \ol{f(x_1, \dots, x_k)}, \ol{z_1}, \dots, \ol{z_l}) \]
            Per un lemma visto tutti gli elementi non standard sono maggiori di ogni numerale, per cui $N$ è maggiore di $\{\ldots\}_M f(x_1, \dots, x_k)$ e di $\{\ldots\}_M z_1, \dots, \{\ldots\}_M z_l$.
            Quindi i numerali che abbiamo scelto rispettano le disuguaglianze e soddisfano $\psi$, per cui, dovendo $M$ soddisfare $\theta_2$, si ottiene che $M \models y = \ol{f(x_1, \dots, x_k)}$, assurdo.
        \end{itemize}
        \item[$\boxed{\leftarrow}$] Vogliamo mostrare che $\mathsf{Q} \vdash \forall y \, (\varphi(\ol{x_1}, \dots, \ol{x_k}, y) \leftarrow y = \ol{f(x_1, \dots, x_k)})$.\\
        Ossia, per El$_\forall$, $\mathsf{Q} \vdash \varphi(\ol{x_1}, \dots, \ol{x_k}, \ol{f(x_1, \dots, x_k)})$. Dall'equivalenza iniziale, come fatto poco fa, possiamo fissare dei numerali tali che:
        \[ \mathsf{Q} \vdash \psi(\ol{x_1}, \dots, \ol{x_k}, \ol{f(x_1, \dots, x_k)}, \ol{z_1}, \dots, \ol{z_l}) \]
        e chiamiamo $N := \max\{f(x_1, \dots, x_k), z_1, \dots, z_l\}$, nei naturali. Verifichiamo semanticamente la tesi, ossia fissato $M \models \mathsf{Q}$, 
        dobbiamo vedere che in $M$ vale: $M \models \varphi(\ol{x_1}, \dots, \ol{x_k}, \ol{f(x_1, \dots, x_k)})$ con il numerale di $\ol N$ interpretato in $M$.\\
        A questo punto, per quanto riguarda $\theta_1$:
        \[ M \models \exists z_1 \le \ol N \dots \exists z_l \le \ol N \, (\ol{f(x_1, \dots, x_k)} \le \ol N \land \psi(\ol{x_1}, \dots, \ol{x_k}, \ol{f(x_1, \dots, x_k)}, z_1, \dots, z_l)) \]
        è sufficiente prendere come $z_i$ proprio l'interpretazione dei numerali fissati all'inizio, che rispettano le disuguaglianze, e con questa scelta $M$ soddisfa $\psi$, perché si deduce da $\mathsf{Q}$ con quei parametri.
        Per quanto riguarda $\theta_2$ vogliamo:
        \[ M \models \forall z_1 \le \ol N \dots \forall z_l \le \ol N \, \forall t \le \ol N \, (\psi(\ol{x_1}, \dots, \ol{x_k}, t, z_1, \dots, z_l) \to t = \ol{f(x_1, \dots, x_k)}) \]
        ossia, dati $z_1, \dots, z_l, t \in M$ tali che:
        \[ M \models z_1 \le \ol N \land \ldots \land z_l \le \ol N \land t \le \ol N \land \psi(\ol{x_1}, \dots, \ol{x_k}, t, z_1, \dots, z_l) \]
        deve valere necessariamente che $\purple{M \models t = \ol{f(x_1, \dots, x_k)}}$. Però, le disuguaglianze all'inizio, essendo $\ol N$ numerale, implicano per un lemma,
        che $z_1, \dots, z_l, t$ sono standard. Quindi l'ipotesi diventa:
        \[ M \models \psi(\ol{x_1}, \dots, \ol{x_k}, \ol t, \ol{z_1}, \dots, \ol{z_l}) \]
        che, per assolutezza, è equivalente a $\mathsf{Q} \vdash \psi(\ol{x_1}, \dots, \ol{x_k}, \ol t, \ol{z_1}, \dots, \ol{z_l})$, ovvero,
        per il lemma iniziale $f(x_1, \dots, x_k) = t$, quindi il numerale di $f(...)$ è proprio $\ol t$, e quindi (volendo si passa per il lemma per cui $\mathsf{Q}$ sa fare i conti) $M \models t = \ol{f(x_1, \dots, x_k)}$.
    \end{itemize}
\end{proof}
...
\begin{definition}[Codifica di termini e formule]
    ...
    \begin{itemize}
        ...
        \[ \ulcorner 0\urcorner := \langle\#0\rangle \qquad \ulcorner s(t)\urcorner := \langle\#s,\ulcorner t\urcorner\rangle \]
        \[ \ulcorner x_i\urcorner := \langle\#x_i\rangle \]
        \[ \ulcorner t_1+t_2\urcorner := \langle\#+,\ulcorner t_1\urcorner,\ulcorner t_2\urcorner\rangle \qquad \ulcorner t_1\cdot t_2\urcorner := \langle\#\cdot,\ulcorner t_1\urcorner,\ulcorner t_2\urcorner\rangle \]
        \item $L_{\text{arit}}$-formule:
        \[ \ulcorner t_1=t_2\urcorner := \langle\#=,\ulcorner t_1\urcorner,\ulcorner t_2\urcorner\rangle\qquad \]
...
Abbiamo così ottenuto una numerazione iniettiva di ogni $L_{\text{arit}}$-termine e di ogni $L_{\text{arit}}$-formula. In particolare le mappe $\cdot \mapsto \ulcorner\cdot\urcorner$ sono iniettive e PR sia per i termini che per le formule.
Il nostro obiettivo sarà dimostrare il seguente lemma.

\begin{lemma}[Lemma di diagonalizzazione di Gödel]
    Sia $\varphi(x)$, con $x \in \vl(\varphi)$, una $L_{\text{arit}}$-formula, allora esiste una $L_{\text{arit}}$-formula $\textcolor{purple}{\psi}$ tale che:
    \[ \mathsf{Q} \vdash \textcolor{purple}{\psi} \leftrightarrow \varphi[\overline{\ulcorner\textcolor{purple}{\psi}\urcorner}/x] \]
\end{lemma}

\textcolor{MidnightBlue}{Cioè per ogni formula con una variabile libera ce n'è un'altra che è vera se e solo se è vera ``$\varphi(\psi)$".}
Di fatto il lemma ci dà una specie di teorema di punto fisso per le formule non chiuse - sennò la tesi è banale; cioè per ogni formula ne esiste un'altra 
...
La proprietà che per cui ci interesserà usare il lemma è ``io sono dimostrabile''.
...
\begin{proof}
    Per decidere se $n$ è codice di una formula, basta verificare che il primo simbolo della lista codificata dal numero corrisponda ad un simbolo di formula e controllare che i restanti simboli siano codici di sottoformule o di termini, a seconda del simbolo iniziale.
    Iniziamo definendo un predicato PR $\textsf{Term}(n)$ che dice se $n$ è codice di un termine, e poi usiamo questo predicato per definire $\textsf{For}(n)$. La definizione di $\textsf{Term}(n)$ la si dà
    per ricorsione sulla lunghezza della lista codificata da $n$ e la diamo usando i predicati PR in riferimento agli insiemi di confronto che definiscono:
    \begin{align*}
        \textsf{Term}(n) :=& (\length(n) = 1 \land \nth(n,0) = \#0) \\
                           & \lor (\length(n) = 1 \land \nth(n,0) = \#x_i) \\
                           &\lor (\length(n) = 2 \land \nth(n,0) = \#s \land \textsf{Term}(\nth(n,1))) \\
                           &\lor (\length(n) = 3 \land \nth(n,0) = \#+ \land \textsf{Term}(\nth(n,1)) \land \textsf{Term}(\nth(n,2))) \\
                           &\lor (\length(n) = 3 \land \nth(n,0) = \#\cdot \land \textsf{Term}(\nth(n,1)) \land \textsf{Term}(\nth(n,2)))
    \end{align*}
    A questo punto, usando $\textsf{Term}(n)$, possiamo definire $\textsf{For}(n)$ sempre per ricorsione sulla lunghezza:
    \begin{align*}
        \textsf{For}(n) :=& (\length(n) = 1 \land \nth(n,0) = \#\bot) \\
                          &\lor (\length(n) = 3 \land \nth(n,0) = \#= \land \textsf{Term}(\nth(n,1)) \land \textsf{Term}(\nth(n,2))) \\
                          &\lor (\length(n) = 3 \land \nth(n,0) = \#\to \land \textsf{For}(\nth(n,1)) \land \textsf{For}(\nth(n,2))) \\
...
\begin{proof}
    Si può definire $\textsf{Num}(n)$ per ricorsione su $n$ in modo PR:
...
\begin{proof}
    ...
    \[ \textsf{Sub}(n,i,x) := \begin{cases}
        0 & \text{se $\neg\,\textsf{For}(n)$} \\
        \langle \#\bot \rangle & \text{se $\nth(n,0) = \#\bot$} \\
        \langle \#=, \textsf{Subterm}(\nth(n,1),i,x), \textsf{Subterm}(\nth(n,2),i,x) \rangle & \text{se $\nth(n,0) = \#=$} \\
        \langle \#\to, \textsf{Sub}(\nth(n,1),i,x), \textsf{Sub}(\nth(n,2),i,x) \rangle & \text{se $\nth(n,0) = \#\to$} \\
        n & \text{se $\nth(n,0) = \#\forall$ e $\nth(n,1) = i + 8$} \\
        \langle \#\forall, \nth(n,1), \textsf{Sub}(\nth(n,2),i,x) \rangle & \text{se $\nth(n,0) = \#\forall$ e $\nth(n,1) \ne i + 8$}
    \end{cases} \]
    dove $\textsf{Subterm}$ è la funzione che fa la sostituzione nei termini, definita come:
    \[ \textsf{Subterm}(n,i,x) := \begin{cases}
        0 & \text{se $\neg\,\textsf{Term}(n)$} \\
        \langle\#0\rangle & \text{se $\nth(n,0) = \#0$} \\
        \langle\#s, \textsf{Subterm}(\nth(n,1),i,x)\rangle & \text{se $\nth(n,0) = \#s$} \\
        \langle\#+, \textsf{Subterm}(\nth(n,1),i,x), \textsf{Subterm}(\nth(n,2),i,x)\rangle & \text{se $\nth(n,0) = \#+$} \\
        \langle\#\cdot, \textsf{Subterm}(\nth(n,1),i,x), \textsf{Subterm}(\nth(n,2),i,x)\rangle & \text{se $\nth(n,0) = \#\cdot$} \\
        x & \text{se $\nth(n,1) = i + 8$} \\
        n & \text{se $\nth(n,1) \ne i + 8$}
    \end{cases} \]
\end{proof}

Vediamo ora la dimostrazione del lemma di diagonalizzazione di Gödel.\\
...
\begin{proof}
    Vogliamo dimostrare che data $\varphi(x_1)$, con $x_1 \in \vl(\varphi)$, esiste $\psi$ tale che $\mathsf{Q} \vdash \psi \leftrightarrow \varphi[\overline{\ulcorner\psi\urcorner}/x_1]$.
    Sia $x_k \,\purple{\not\in} \,\vl(\varphi)$ e definiamo la funzione $f$ PR come:
    \[ f : \NN \to \NN : x \mapsto \mathsf{Sub}(x,k,\textsf{Num}(x)) \]
    ovvero la funzione che sostituisce, alla variabile non libera $x_k$, il numerale corrispondente al numero di Gödel della formula codificata da $x$, ovvero
    $\ulcorner \alpha \urcorner \mapsto \ulcorner \alpha[\ol{\ulcorner\alpha\urcorner}/x_k]\urcorner$ - possiamo pensare a questa cosa come $\alpha(\alpha)$.
    Sia $\pi$ una formula con (WLOG a meno di cambiare $x_k$) $\vl(\pi) = \{x_1,x_k\}$ tale che:
    \[ \forall x \in \NN \quad \mathsf{Q} \vdash \forall x_1 \, \pi(x_1,\ol x/x_k) \leftrightarrow x_1 = \ol{f(x)} \]
    tale formula esiste per il lemma di rappresentazione di funzioni ricorsive totali e metto il valore in input di $f$ al posto della variabile $x_k$.
    Definiamo $\theta := \purple{\forall x_1 \,\pi \to \varphi}$ e $\psi := \purple{\theta[\ulcorner\theta\urcorner/x_k]}$ - cioè $\ulcorner \psi \urcorner = f(\ulcorner\theta\urcorner)$, e quindi $\psi$ è proprio $\theta$ applicata a se stessa.\\
    Ora verifichiamo la tesi, usando $x = \ulcorner\theta\urcorner$, per il lemma abbiamo:
    \[ \mathsf{Q} \vdash \forall x_1 \, \pi(x_1,\ol{\ulcorner\theta\urcorner}/x_k) \leftrightarrow x_1 = \ol{f(\ulcorner\theta\urcorner)} \]
    e per definizione $f(\ulcorner\theta\urcorner) = \ulcorner\psi\urcorner$, quindi possiamo riscrivere l'equivalenza come:
    \[ \mathsf{Q} \vdash \forall x_1 \, \pi(x_1,\ol{\ulcorner\theta\urcorner}/x_k) \leftrightarrow x_1 = \ol{\ulcorner\psi\urcorner} \]
    Riscrivendo $\psi$ esplicitamente:
    \begin{align*}
        \psi &= \theta[\ulcorner\theta\urcorner/x_k] \\
             &= (\forall x_1 \, \pi \to \varphi)[\ulcorner\theta\urcorner/x_k] \\
             &= \forall x_1 \, \pi[\ulcorner\theta\urcorner/x_k] \to \varphi \\
             &= \forall x_1 \, \pi(x_1,\ol{\ulcorner\theta\urcorner}/x_k) \to \varphi \\
             &\overset{\mathsf{Q}}{\leftrightarrow} x_1 = \ol{\ulcorner\psi\urcorner} \to \varphi \\
             &\leftrightarrow \varphi[\ol{\ulcorner\psi\urcorner}/x_1]
    \end{align*}
...
\begin{definition}[Teoria ricorsivamente - o semidecidibilmente - assiomatizzabile]
    ...
    \[ \{\ulcorner \sigma \urcorner : \sigma \in T\} = \{\ulcorner\sigma\urcorner : P(\ulcorner\sigma\urcorner) \ne \bot \} \]
    con $P$ una funzione ricorsiva parziale, ovvero \purple{esiste un algoritmo che si ferma se e solo se la formula in input è un assioma di $T$}.
\end{definition}

In realtà, per via di un trucco, possiamo sempre supporre che l'insieme di assiomi sia addirittura decidibile.

\begin{definition}[Teoria decidibilmente assiomatizzabile]
    Una teoria $T$ è \vocab{decidibilmente assiomatizzabile} se l'insieme dei codici degli assiomi di $T$ è decidibile, ovvero esiste $P$ funzione ricorsiva totale tale che:
    \[ \{\ulcorner \sigma \urcorner : \sigma \in T\} = \{\ulcorner\sigma\urcorner : P(\ulcorner\sigma\urcorner) = 0 \} \]
    in altre parole, \textcolor{purple}{esiste un algoritmo che si ferma sempre, e dice 0 se e solo se la formula in input è un assioma di $T$}.
\end{definition}

\begin{exercise}[Teorema di Craig]
    Sia $T$ ricorsivamente assiomatizzabile, allora esiste una teoria decidibilmente assiomatizzabile, tale che $T$ e $T'$ sono equivalenti.
\end{exercise}
...
\begin{soln}
    ...
    Inoltre, se poniamo $h(n) := \ulcorner\varphi_n'\urcorner$, la funzione $h : \NN \to \NN$ è ricorsiva totale (si ottiene da $f$ componendo con operazioni sintattiche ricorsive, cioè codifica e congiunzione iterata\footnote{Se avessimo voglia di scrivere
    la funzione PR $\textsf{Conj}(a,b) := \ulcorner\varphi_a \land \varphi_b\urcorner$, di definire $H(1,k) := k, H(n+1,k) := \textsf{Conj}(H(n,k),k)$ e di definire $h(n) := H(n,f(n))$, e quindi $h$ ricorsiva totale. Si potrebbe fare meglio ed ottenere PR, ma non ho voglia.}).
    Quindi possiamo definire una funzione ricorsiva totale $g$ che controlla se un elemento è nell'immagine di $h$,
    moralmente facendone una ``ricerca limitata'':
    \[ g(z) := \begin{cases}
        0 & \text{se $\exists m \leq z \; h(m) = z$} \\
        1 & \text{altrimenti}
    \end{cases} \]
    che è totale e vale che $T' = \{z : g(z) = 0\}$.
    \textcolor{MidnightBlue}{Moralmente abbiamo codificato nell'assioma stesso l'$x$ che codice quell'assioma, ripetendolo l'AND $x$ volte, per cui nella ricerca si ha che \purple{alla peggio si arriva a $x$ e si trova l'assioma, e alla meglio lo si trova prima}.}
\end{soln}

\begin{notation}
    D'ora in poi, intenderemo equivalentemente le espressioni ricorsivamente assiomatizzabile e decidibilmente assiomatizzabile. 
\end{notation}

\begin{lemma}[Predicato di dimostrazione]
    Sia $T$ una $L_{\text{arit}}$-teoria ricorsivamente assiomatizzabile, allora esiste un predicato PR $\mathsf{Dim}_T(x,y)$ tale che:
...
\begin{proof}
    In primis diciamo come è codificata una dimostrazione: dire che $T \vdash \varphi$ significa che esiste una sequenza finita di sequenti $\sigma_1, \sigma_2, \ldots, \sigma_n$, con $\sigma_n = T \vdash \varphi$, e per ogni $i < n$, $\sigma_i$ si ottiene dai precedenti usando le regole di inferenza del sistema di deduzione naturale $\textsf{ND}$, in questo caso
    usiamo $\textsf{ND}_{\to,\bot,\forall,=}$, per comodità (è equivalente a quello usato in precedenza). Inoltre ogni sequente $S \vdash \psi$ indica per definizione che esiste un sottoinsieme finito di formule di $S$ dalle quali si arriva a $\psi$ iterando, per un numero finito di passaggi le regole di inferenza.\\
    A questo punto ha senso codificare un sequente $S \vdash \psi$ come la lista dell'insieme delle formule $\varphi_1, \ldots, \varphi_n$ usate per ottenere $\psi$:
    \[ \ulcorner S \vdash \psi\urcorner := \langle \langle \ulcorner \varphi_1\urcorner, \ulcorner\varphi_2\urcorner, \ldots, \ulcorner\varphi_n\urcorner\rangle, \ulcorner\psi\urcorner\rangle \]
    e quindi naturalmente possiamo codificare una dimostrazione come la lista dei codici dei sequenti:
    \[ \ulcorner \sigma_1, \sigma_2, \ldots, \sigma_n\urcorner := \langle \ulcorner\sigma_1\urcorner, \ulcorner\sigma_2\urcorner, \ldots, \ulcorner\sigma_n\urcorner\rangle \]
    Ora possiamo definire il predicato PR $\mathsf{Dim}_T(d,x)$, che dice ``$d$ è un codice di una dimostrazione di $x$ in $T$'' traducendo il significato della definizione di dimostrazione in una definizione PR:
    \begin{align*}
        \textsf{Dim}_T(d,x) &:= \nth(d,\length(d) \dotminus 1) = x \\
                            &\land \forall i < \length(d) \; \text{``$\nth(d,i)$ è o un assioma di $T$ o segue da quelli prima...''}
    \end{align*}
    cioè la dimostrazione finisce con la formula codificata da $x$, e ogni passo della dimostrazione è o un assioma o segue da quelli prima usando le regole di inferenza.\\
    ``$\nth(d,i)$ è un assioma'' è esprimibile come predicato PR, perché per ipotesi $T$ è ricorsivamente assiomatizzabile, quindi esiste una funzione ricorsiva totale - anzi PR - da mettere in quel posto.
    ``$\nth(d,i)$ segue dalle formule prima usando le regole di inferenza'' basta scrivere un lungo OR che esprima, per tutte le possibili regole di inferenza, che $\nth(d,i)$ si ottiene da quella regola applicata a formule che appaiono prima, ad esempio:
    \[ \text{In}_{\to} \frac{T \vdash \varphi \quad T \vdash \varphi \to \psi}{T \vdash \psi} \]
    equivale a:
    \[ \exists j < i \; \exists t < i \; \underbrace{\nth(d,t)}_{= \varphi \to \psi} = \textsf{implica}(\underbrace{\nth(d,j)}_{= \varphi}, \underbrace{\nth(d,i)}_{= \psi}) \]
    dove $\textsf{implica}$ è la funzione PR che prende i codici di due formule e restituisce il codice della formula che è l'implicazione tra le due - cioè $\langle \#\to, \nth(d,j), \nth(d,i)\rangle$ - e questo è il modo giusto di dire che $\nth(d,i)$ si ottiene da El$_\to$.
    Analogamente gli altri casi, facendo attenzione al caso di In$_\forall$ dove bisogna controllare che la variabile libera sia effettivamente libera nelle premesse, e quindi introdurre $\textsf{free}(y,i) :=$ ``$x_i$ è libera in $y$''.
\end{proof}

\begin{remark}
    Notare: abbiamo usato un solo $\nth$ come se una dimostrazione fosse una lista di formule, ma in realtà è una lista di sequenti, quindi per essere precisissimi,
    dovremmo aggiungere un secondo layer di $\nth$ per accedere alla formula finale del sequente.
\end{remark}

\begin{notation}
    Abbiamo definito $\mathsf{Dim}_T(d,x)$ come predicato PR, ma lo useremo equivalentemente anche per esprimere la formula associata al predicato, che esprime ``$d$ è un codice di una dimostrazione di $x$ in $T$'',
    che è di classe $\Sigma_1^0$.
\end{notation}

\begin{remark}
    La formula $\textsf{free}(n,i)$, che dice che $x_i$ è libera in $n$, è un predicato PR che può essere definito per ricorsione - sulla lunghezza della formula codificata - tuttavia ci serve prima una formula 
    apposta per controllare se la variabile è libera o meno in un $L_{\text{arit}}$-termine, e possiamo definirla come segue:
    \begin{align*}
        \textsf{free\_term}(n,i) :=& (\length(n) = 1 \land \nth(n,0) = \#x_i) \\
                             &\lor (\length(n) = 2 \land \nth(n,0) = \#s \land \textsf{free}(\nth(n,1),i)) \\
                             &\lor (\length(n) = 3 \land \nth(n,0) = \#+ \land (\textsf{free}(\nth(n,1),i) \lor \textsf{free}(\nth(n,2),i))) \\
                             &\lor (\length(n) = 3 \land \nth(n,0) = \#\cdot \land (\textsf{free}(\nth(n,1),i) \lor \textsf{free}(\nth(n,2),i)))
    \end{align*}
    a questo punto, usando $\textsf{free\_term}$, possiamo definire $\textsf{free}(n,i)$:
    \begin{align*}
        \textsf{free}(n,i) :=& (\length(n) = 3 \land \nth(n,0) = \#= \land (\textsf{free\_term}(\nth(n,1),i) \lor \textsf{free\_term}(\nth(n,2),i))) \\
                             &\lor (\length(n) = 3 \land \nth(n,0) = \#\to \land (\textsf{free}(\nth(n,1),i) \lor \textsf{free}(\nth(n,2),i))) \\
                             &\lor (\length(n) = 3 \land \nth(n,0) = \#\forall \land (\textsf{free}(\nth(n,2),i) \land \nth(n,1) \ne i + 8))
    \end{align*}
\end{remark}
...
\textcolor{MidnightBlue}{Lo scopo è costruire una formula $\gamma$ che dica ``io non sono dimostrabile in $T$'', ovvero una formula che per $T$ è proprio equivalente al fatto che non esiste una dimostrazione per lei.}

\begin{proof}
    Sia $\varphi(x) := \neg\,\exists d \, \mathsf{Dim}_T(d,x)$, ovvero ``$x$ non è dimostrabile per $T$'', usando il lemma di diagonalizzazione di Gödel troviamo $\gamma$ tale che:
    \[ \mathsf{Q} \vdash \gamma \leftrightarrow \neg\,\exists d \, \mathsf{Dim}_T(d,\overline{\ulcorner\gamma\urcorner}) \]
    distinguiamo due casi.
    \begin{itemize}
        \item Assumiamo $T \vdash \gamma$, allora:
        \begin{align*}
            T \vdash \gamma &\iff \NN \models \exists d \, \mathsf{Dim}_T(d,\ulcorner\gamma\urcorner) &&(\text{lemma}) \\
                            &\iff \mathsf{Q} \vdash \exists d \, \mathsf{Dim}_T(d,\overline{\ulcorner\gamma\urcorner}) &&(\Sigma_1^0) \\
                            &\iff \mathsf{Q} \vdash \neg\,\gamma && (\text{equivalenza di $\gamma$}) \\
                            &\implies T \vdash \neg\,\gamma \; \lightning && (T \vdash \mathsf{Q})
        \end{align*} 
        \item Assumiamo $T \vdash \neg\,\gamma$, allora:
        \begin{align*}
            T \vdash \neg\,\gamma &\iff T \vdash \exists d \, \mathsf{Dim}_T(d,\overline{\ulcorner\gamma\urcorner}) && (\text{equivalenza di $\gamma$}) \\
                                   &\implies \NN \models \exists d \, \mathsf{Dim}_T(d,\ulcorner\gamma\urcorner) && (\NN \models T) \\
                                   &\iff T \vdash \gamma \; \lightning && (\text{lemma})
        \end{align*}
    \end{itemize}
\end{proof}
...
\begin{proof}
    L'idea della dimostrazione, è quella di usare una formula che dice ``se ci fosse una mia dimostrazione, allora ci sarebbe anche una dimostrazione di una mia negazione più corta'', ovvero:
    \[ \varphi(x) := \forall d \, (\mathsf{Dim}_T(d,x) \to \exists d' \leq d \, \mathsf{Dim}_T(d',\textsf{Neg}(x))) \]
    con $\textsf{Neg}(x) = \langle \#\neg\,, x\rangle$\footnote{In realtà andrebbe codificata come $x \to \bot$, e quindi $\langle \#\to, x, \ulcorner \bot \urcorner \rangle$.} la funzione PR che associa ad una formula il codice della sua negazione. Usiamo il lemma di diagonalizzazione di Gödel su questa formula e troviamo $\gamma$ tale che:
    \[ \mathsf{Q} \vdash \gamma \leftrightarrow \forall d \, (\mathsf{Dim}_T(d,\overline{\ulcorner\gamma\urcorner}) \to \exists d' \leq d \, \mathsf{Dim}_T(d',\textsf{Neg}(\overline{\ulcorner\gamma\urcorner}))) \]
    Distinguiamo due casi.
    \begin{itemize}
        \item Assumiamo $T \vdash \gamma$, per coerenza si ha $T \nvdash \neg\, \gamma$, da cui per il lemma valgono:
        \[ \NN \models \exists d \, \mathsf{Dim}_T(d,\ulcorner\gamma\urcorner) \quad \text{e} \quad \NN \models \neg\,\exists d' \, \mathsf{Dim}_T(d',\textsf{Neg}(\ulcorner\gamma\urcorner)) \]
        in particolare:
        \[ \exists k \in \NN \; \NN \models \mathsf{Dim}_T(k,\ulcorner\gamma\urcorner) \quad \text{e} \quad \forall k' \in \NN \; \NN \models \neg\,\mathsf{Dim}_T(k',\textsf{Neg}(\ulcorner\gamma\urcorner)) \]
        e la seconda cosa si può indebolire a $\forall k' \leq k \; \NN \models \ldots$, che equivale a $\NN \models \forall d' \leq \ol k \, \neg\,\mathsf{Dim}_T(d',\textsf{Neg}(\ulcorner\gamma\urcorner))$, quindi:
        \[ \exists k \in \NN \; \NN \models (\mathsf{Dim}_T(\ol k,\ulcorner\gamma\urcorner) \land \forall d' \leq \ol k \, \neg\,\mathsf{Dim}_T(d',\textsf{Neg}(\ulcorner\gamma\urcorner))) \]
        la formula a destra è chiusa e di classe $\Sigma_1^0$, quindi equivale:
        \[ \exists k \in \NN \; \mathsf{Q} \vdash (\mathsf{Dim}_T(\ol k,\ol{\ulcorner\gamma\urcorner}) \land \forall d' \leq \ol k \, \neg\,\mathsf{Dim}_T(d',\textsf{Neg}(\ol{\ulcorner\gamma\urcorner}))) \]
        usando In$_\exists$ troviamo:
        \[ \mathsf{Q} \vdash \exists d \, (\mathsf{Dim}_T(d,\ol{\ulcorner\gamma\urcorner}) \land \forall d' \leq d \, \neg\,\mathsf{Dim}_T(d',\textsf{Neg}(\ol{\ulcorner\gamma\urcorner}))) \]
        che equivale a $\neg\,\gamma$ per il lemma, per cui si ottiene $T \vdash \neg\,\gamma$, assurdo.
        \item Assumiamo $T \vdash \neg\,\gamma$, come prima, per la coerenza si arriva a:
        \[\exists k \in \NN \; \mathsf{Q} \vdash \mathsf{Dim}_T(\ol k,\ol{\mathsf{Neg}(\ulcorner\gamma\urcorner)}) \land \forall d' \leq \ol k \, \neg\,\mathsf{Dim}_T(d',\ol{\ulcorner\gamma\urcorner}) \]
        usando El$_{\cdot \land}$ e El$_{\land \cdot}$ troviamo:
        \[ \exists k \in \NN \quad \begin{cases}
            \mathsf{Q} \vdash \mathsf{Dim}_T(\ol k,\ol{\mathsf{Neg}(\ulcorner\gamma\urcorner)}) \\
            \forall d' \leq \ol k \, \mathsf{Q} \vdash \neg\,\mathsf{Dim}_T(d',\ol{\ulcorner\gamma\urcorner})
        \end{cases}\]
        Vale la dicotomia dell'ordinamento di $\mathsf{Q}$, per cui $\mathsf{Q}\vdash \forall d (d \leq \ol k \lor \ol k \leq d)$, e quindi distinguiamo due casi.
        \begin{itemize}
            \item Se $d \leq \ol k$ allora per la seconda condizione sopra vale $\mathsf{Q} \vdash \neg\,\mathsf{Dim}_T(d,\ol{\ulcorner\gamma\urcorner})$;
            \item se $\ol k \leq d$ allora, la prima condizione sopra dice $\mathsf{Q} \vdash \mathsf{Dim}_T(\ol k,\ol{\mathsf{Neg}(\ulcorner\gamma\urcorner)})$, essendo $\ol k \leq d$,
            possiamo introdurre la quantificazione esistenziale limitata (usando In$_\exists$ e In$_\land$) e ottenere $\mathsf{Q} \vdash \exists d' \leq d \, \mathsf{Dim}_T(d',\ol{\mathsf{Neg}(\ulcorner\gamma\urcorner)})$.
        \end{itemize}
        Pertanto, per El$_{\lor}$ siamo arrivati a:
        \[ \mathsf{Q} \vdash \forall d \, (\neg\,\mathsf{Dim}_T(d,\ol{\ulcorner\gamma\urcorner}) \lor \exists d' \leq d \, \mathsf{Dim}_T(d',\ol{\mathsf{Neg}(\ulcorner\gamma\urcorner)})) \]
        che equivale a:
        \[\mathsf{Q} \vdash \forall d \, (\mathsf{Dim}_T(d,\ol{\ulcorner\gamma\urcorner}) \to \exists d' \leq d \, \mathsf{Dim}_T(d',\ol{\mathsf{Neg}(\ulcorner\gamma\urcorner)})) \]
        che equivale a $\gamma$ per l'equivalenza iniziale, e segue l'assurdo $T \vdash \gamma$.
    \end{itemize}
\end{proof}

Dove si è usato il seguente lemma, che si dimostra per induzione metateorica.

\begin{lemma}[Dicotomia dell'ordinamento di $\mathsf{Q}$]
    $\forall n \in \NN$ vale che $\mathsf{Q} \vdash \forall x (x \leq \ol n \lor \ol n \leq x)$.
\end{lemma}
...
\begin{definition}[Teoria decidibile]
    ...
    analogamente si può definire una teoria \vocab{semidecidibile} usando una funzione ricorsiva parziale.
\end{definition}

Come per l'assiomatizzabilità, l'idea che vogliamo catturare è di un algoritmo che o termina se e solo se la formula in input è dimostrata da $T$, oppure termina sempre e dice 0 se e solo se la formula in input è dimostrata da $T$.

\begin{remark}[Ricorsivamente assiomatizzabile $\implies$ semidecidibile]
    Se $T$ è ricorsivamente assiomatizzabile, allora abbiamo il predicato PR $\mathsf{Dim}_T$, che è quindi ricorsivo totale. Per cui posso definire i codici delle conseguenze di $T$ come:
    \[ \{\ulcorner\sigma\urcorner : T \vdash \sigma\} = \{\ulcorner\sigma\urcorner : \exists d \, \mathsf{Dim}_T(d,\ol{\ulcorner\sigma\urcorner})\} \]
    dove sto considerando la formula, di classe $\Sigma_1^0$, associata a esistenziale + predicato di dimostrazione, e quindi $T$ è decidibilmente assiomatizzabile.
    Ovviamente varrà anche che se $T$ è decidibilmente assiomatizzabile, allora $T$ è semidecidibile.
\end{remark}

\begin{proposition}[Completamento decidibile]
    Sia $T$ una $L_{\text{arit}}$-teoria coerente e decidibile, allora esiste un'estensione coerente $T'$ di $T$ che è completa e decidibile.
\end{proposition}

\begin{proof}
    Definiamo la seguente successione di $L_{\text{arit}}$-teorie:
    \[ T_0 := T \]
    \[ T_{i+1} := \begin{cases}
        T_i \cup \{\varphi\} & \text{se $i = \ulcorner\varphi\urcorner$ e $T_i \nvdash \neg\,\varphi$} \\
        T_i \cup \{\top\} & \text{altrimenti}
    \end{cases} \]
    definiamo $T' := \bigcup_{i \in \NN} T_i$ e vediamo che è coerente, completa e decidibile.
    \begin{itemize}
        \item $T'$ è \emph{coerente}: se $T'$ fosse incoerente, allora per compattezza potremmo ridurci a $T_{i_0} \vdash \bot$ per qualche $i_0$; osserviamo tuttavia che 
        $T_i$ è coerente per ogni $i$: $T_0$ è coerente per ipotesi, e se $T_i$ è coerente, allora se $T_{i+1} = T_i \cup \{\top\}$ questa è in automatico coerente,
        altrimenti $T_{i+1} = T_i \cup \{\varphi\} \vdash \bot$, quindi per In$_\neg\,$ $T_i \vdash \neg\,\varphi$, assurdo per la definizione di $T_{i+1}$.
        \item $T'$ è \emph{completa}: ogni formula chiusa $\varphi$ è codificata da un numero $i$, a questo punto si danno due casi: o $T_i \nvdash \neg\,\varphi$, e allora $T_{i+1} = T_i \cup \{\varphi\}$, quindi $T' \vdash \varphi$;
        oppure $T_i \vdash \neg\,\varphi$, e allora $T' \vdash \neg\,\varphi$.
        \item $T'$ è \emph{decidibile}: per ipotesi $T$ è decidibile, quindi esiste $P$ ricorsiva totale tale che $P(i) = 0$ se e solo se $T$ dimostra la formula codificata da $i$. A questo punto, per ogni formula $\varphi$, preso $k = \ulcorner\varphi\urcorner$,
        per stabilire se $T' \vdash \varphi$ basta controllare se $T_k \nvdash \neg\,\varphi$, che equivale a $T \cup \{\psi_1, \ldots, \psi_n\} \nvdash \neg\,\varphi$, ovvero, per In$_\to$:
        \[ T \nvdash (\psi_1 \land \ldots \land \psi_n) \to \neg\,\varphi \]
        preso il codice della cosa destra di $\vdash$ è sufficiente controllare che $P$ di quel codice sia diverso da 0, in tal modo $T' \vdash \varphi$. Controllare che $P(m) \ne 0$ è decidibile in quanto è il complementare di $\{P(i) = 0\}$, quindi esisterà una funzione decidibile di cui è luogo di zero ($1 \dotminus P(i)$). 
    \end{itemize}
\end{proof}
\pagebreak
\begin{remark}[Completamento decidibile in un qualsiasi linguaggio finito]
    Notare che nella proposizione precedente non abbiamo usato nulla di $L_{\text{arit}}$, se non il fatto che sia un linguaggio finito, per cui posso fissare una numerazione di Gödel.
    Quindi la proposizione è vera per qualsiasi $L$ finito, adattando le definizioni di decidibilità e assiomatizzabilità al linguaggio $L$.
\end{remark}

\begin{remark}[Semidecidibile $\implies$ ricorsivamente assiomatizzabile]
    Se $T$ è semidecidibile, allora imitando esattamente la dimostrazione del teorema di Craig, possiamo costruire una teoria $T'$ decidibilmente assiomatizzabile, tale che $T$ e $T'$ sono equivalenti, e quindi $T$ è ricorsivamente assiomatizzabile.
\end{remark}

Vediamo tre facili applicazioni dell'ultima cosa.
...
\begin{proof}
    Se $\mathsf Q$ fosse decidibile, allora, per la proposizione precedente, esisterebbe un'estensione coerente, completa e decidibile di $\mathsf Q$.
    In particolare, tale estensione sarebbe ricorsivamente assiomatizzabile, e questo è impossibile per il teorema di Rosser. Stesso identico discorso per $\mathsf{PA}$, che estende $\mathsf Q$.
\end{proof}
...
\begin{proof}
    Supponiamo per assurdo che sia decidibile l'insieme:
    \[ \{\ulcorner\sigma\urcorner : \;\vdash \sigma\} \]
    osserviamo che l'insieme, che abbiamo visto essere non decidibile, dei codici delle formule vere in $\mathsf{Q}$ si scrive come:
    \[ \{\ulcorner\varphi\urcorner : \mathsf{Q} \vdash \varphi\} = \{\ulcorner\varphi\urcorner : \;\vdash (\mathsf{Q}1 \land \ldots \land \mathsf{Q}7) \to \varphi\} \]
    Per cui se ci fosse un algoritmo che decide se una formula è dimostrabile senza assiomi, allora potremmo usarlo per decidere se una formula è dimostrata da $\mathsf{Q}$, assurdo.
\end{proof}
...
\begin{proof}
    Tale insieme è proprio l'insieme dei codici delle formule in $\Th(\NN;L_{\text{arit}})$, se fosse decidibile sarebbe un'estensione coerente, completa e ricorsivamente assiomatizzabile di $\mathsf{Q}$, assurdo per il primo teorema di incompletezza.
\end{proof}

\begin{proposition}[Completezza + semidecidibilità $\implies$ decidibilità]
    Una teoria $T$ completa e semidecidibile è decidibile.
\end{proposition}

\begin{proof}
    Per ipotesi $T$ è semidecidibile, quindi esiste una funzione ricorsiva parziale $P$ tale che $P(i) = 0$ se e solo se $T \vdash \sigma$, dove $i = \ulcorner\sigma\urcorner$.
    Definiamo questi tre insiemi:
    \[ A := \{n \in \NN : ``T \vdash n\text{''} \land \mathsf{Form}(n) = 0\} \]
    \[ B := \{n \in \NN : ``T \vdash \neg\, \mathsf{Cl}(n)\text{''} \land \mathsf{Form}(n) = 0\} \]
    \[ C := \{n \in \NN : \mathsf{Form}(n) = 1\} \]
    la suddivisione viene fatta dividendo prima i codici di formule da quelli di non-formule, e poi dividendo i codici di formule in quelli dimostrati da $T$ e quelli le cui negazioni sono dimostrate da $T$,
    in particolare, se non vale che $T \vdash \varphi$, visto che è equivalente a dimostrare la chiusura universale di $\varphi$, allora deve avvenire che $T$ dimostri la sua negazione,
    tuttavia se $\varphi$ è aperta, la negazione da dimostrare è quella della chiusura universale, perché $\neg\, \forall \ldots \ne \forall \neg\, \ldots$, cioè $T \nvdash \varphi$ è equivalente a $T \vdash \neg\,\mathsf{Cl}(\varphi)$,
    che è chiusa e quindi è $T \vdash \neg\,\mathsf{Cl}(\varphi)$. Per tale ragione, grazie alla completezza di $T$ quella sopra è una partizione di $\NN$.\\
    Naturalmente $C$ è primitivo ricorsivo, inoltre, scrivendo al posto di ``$T \vdash n$'' il predicato PR di dimostrabilità - che è PR perché $T$ è semidecidibile e quindi ricorsivamente assiomatizzabile - che con l'esistenziale è una formula $\Sigma_1^0$,
    otteniamo che $A$ è semidecidibile, e analogamente verificato che $B$ è semidecidibile, abbiamo che $A$ e $B$ sono decidibili, e quindi $T$ è decidibile.\\
    Ci serve infine verificare che la funzione $\mathsf{Cl}$, che associa ad ogni codice il numero della sua chiusura universale, è PR, infatti basta definirla come:
    $$ g(n, 0) := \begin{cases} \langle \#\forall, 8, n \rangle & \text{se } \mathsf{free}(n, 0) = 0 \\ n & \text{altrimenti} \end{cases} $$
    $$ g(n, k+1) := \begin{cases} \langle \#\forall, (k+1)+8, g(n, k) \rangle & \text{se } \mathsf{free}(n, k+1) = 0 \\ g(n, k) & \text{altrimenti} \end{cases} $$
    e $\mathsf{Cl}(n) := g(n, n)$ (poiché $\cons(x,y) \geq \max{\{x,y\}}$ non posso avere codici in cui la codifica di una variabile superi $n$). Fatto questo $B$ è semidecidibile in quanto la formula che lo definisce è $\Sigma_1^0$.
\end{proof}
\pagebreak
\subsection{Secondo teorema di incompletezza}
Il secondo teorema di incompletezza di Gödel è una conseguenza del primo,
e dice che dentro la teoria $\mathsf{PA}$ non si può dimostrare la coerenza di $\mathsf{PA}$, e più in generale, se $T$ è una teoria aritmetica che estende $\mathsf{Q}$- con qualche proprietà in più, allora $T$ non dimostra la propria coerenza.
...
\begin{notation}[Formula di dimostrabilità]
    ...
    \[ \square \varphi := \exists d\, \mathsf{Dim}_{\mathsf{PA}}(d,\overline{\ulcorner\varphi\urcorner}) \]
    tale abbreviazione è da intendersi per formule chiuse, e per la chiusura universale di formule aperte.
\end{notation}

\begin{remark}
    Con questa notazione, la formula $\gamma$ costruita nel primo teorema di incompletezza di Gödel, nel caso di $T = \mathsf{PA}$, è tale che:
...
\begin{proof}
    Dal primo teorema di incompletezza di Gödel, esiste una formula $\gamma$ che è indecidibile in $\mathsf{PA}$ e tale che:
    \[ \mathsf{Q} \vdash \gamma \leftrightarrow \neg\,\square\gamma \]
    il nostro obiettivo è dimostrare che tale formula è equivalente a $\neg\,\square\bot$ per $\mathsf{PA}$:
    \[ \mathsf{PA} \vdash \gamma \leftrightarrow \neg\,\square\bot \]
    fatto questo si otterrà in particolare che $\neg\,\square\bot$ è indecidibile in $\mathsf{PA}$, e quindi $\mathsf{PA} \nvdash \neg\,\square\bot$.\\
    Visto che $\mathsf{PA} \vdash \mathsf{Q}$, l'equivalenza tra $\gamma$ e $\neg\,\square\gamma$ vale anche in $\mathsf{PA}$, per cui è sufficiente dimostrare l'equivalenza:
    \[ \mathsf{PA} \vdash \neg\,\square\gamma \leftrightarrow \neg\,\square\bot \]
    vediamo le due implicazioni.
    \begin{itemize}
        \item[$\boxed{\neg\,\square\gamma \to \neg\,\square\bot}$]
        Procediamo dimostrando la contronominale $\mathsf{PA} \vdash \square\bot \to \square\gamma$. Usando In$_\to$ vogliamo $\mathsf{PA}, \square\bot \vdash \square\gamma$. Osserviamo in primis che, per il lemma, da $\mathsf{PA}$ seguono due cose:
        \[ (3)\frac{}{\mathsf{PA} \vdash (\square \bot \land \square(\bot \to \gamma)) \to \square\gamma} \]
        inoltre vale sempre che $\mathsf{PA}\vdash \square(\bot \to \gamma)$:
        \[ (1)\frac{\displaystyle \text{In}_\to\frac{\displaystyle \text{Wk}\frac{\displaystyle\text{El}_\bot\frac{\displaystyle \text{Ax}\frac{}{\bot \vdash \bot}}{\bot \vdash \gamma}}{\mathsf{PA}, \bot \vdash \gamma}}{\mathsf{PA} \vdash \bot \to \gamma}}{\mathsf{PA} \vdash \square(\bot \to \gamma)} \]
        a questo punto, per In$\land$ e Wk vale $\mathsf{PA}, \square \bot \vdash \square\bot \land \square(\bot \to \gamma)$, e quindi usando si ottiene la tesi per modus ponens:
        \[ \text{El}_{\to}\frac{\mathsf{PA}, \square \bot \vdash \square\bot \land \square(\bot \to \gamma) \quad \mathsf{PA} \vdash (\square \bot \land \square(\bot \to \gamma)) \to \square\gamma}{\mathsf{PA}, \square\bot \vdash \square\gamma} \]
        \item[$\boxed{\neg\,\square\bot \to \neg\,\square\gamma}$] Come prima dimostriamo la contronominale $\mathsf{PA} \vdash \square\gamma \to \square\bot$.\\
        In primis ci serve un'identità sintattica.
        \begin{remark}
            Data $\theta$ $L$-formula, vale che $\vdash \theta \to (\neg\,\theta \to \bot)$.
        \end{remark}
        Da questa, usando $\square \gamma$ si ottiene:
        \[ (1)\frac{\mathsf{PA} \vdash \square\gamma \to (\neg\,\square\gamma \to \bot)}{\mathsf{PA} \vdash \square(\square\gamma \to (\neg\,\square\gamma \to \bot))} \]
        Ora stiamo assumendo $\mathsf{PA}$ e $\square\gamma$, quindi per (2) vale $\square \square \gamma$, per cui, per (3) si ottiene:
        \[ \mathsf{PA}, \square\gamma \vdash \square(\neg\, \square\gamma \to \bot) \]
        se riuscissimo a riutilizzare (3) avremmo concluso. Abbiamo bisogno di ottenere $\square(\neg\, \square\gamma)$ in qualche modo.\\
        Per il primo teorema di incompletezza sappiamo $\mathsf{PA} \vdash \gamma \to \neg\,\square\gamma$, per cui, usando (1) e poi (3) otteniamo:
        \[ \mathsf{PA}, \square\gamma \vdash \square(\gamma \to \neg\,\square\gamma)  \]
        e si conclude che $\mathsf{PA}, \square\gamma \vdash \square(\neg\,\square\gamma)$, usando (3) si ha l'implicazione.
    \end{itemize}
\end{proof}
\pagebreak
