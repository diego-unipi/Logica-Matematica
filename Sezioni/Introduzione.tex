...
in cui i concetti quali la \vocab{dimostrazione} o la \vocab{relazione di conseguenza logica} vengono concepiti come \purple{oggetti matematici}. In altre parole, la logica diventa matematica nel momento in cui si rende conto che 
...
Cosa abbiamo imparato da questo disastro? Intanto abbiamo dato una definizione precisa di enunciato o \vocab{formula} e una definizione di \vocab{deduzione} basata su regole formali, ossia algebriche, simboliche - ``abbiamo dato'' come comunità matematica, ossia \purple{daremo} durante il corso.
Questa definizione è \purple{quella giusta}, nel senso che ha dimostrato Gödel nel 1929, cioè c'è una nozione associata di \vocab{struttura} - per esempio i gruppi sono precisamente le strutture che soddisfano le formule che esprimono gli assiomi dei gruppi: $\forall x,y,z \; (x \cdot y) \cdot z = x \cdot (y \cdot z)$, $\forall x \; x \cdot e = x$,
...
Fissato il sistema di regole deduttive appena descritto, che chiamiamo \vocab{logica del primo ordine} - con riferimento al fatto che è ammesso quantificare \purple{$\forall x$}, \purple{$\exists x$} su elementi della struttura, ma non si può quantificare sui suoi sottoinsiemi - i teoremi di incompletezza di Gödel constatano 
...
alla materializzazione elettronica di questi concetti. Va da sé che, nel corso, studieremo le basi della \vocab{teoria della computabilità}.\\
Questi argomenti costituiscono quindi l'ossatura tradizionale del corso di logica matematica: \purple{il calcolo dei predicati del primo ordine, i teoremi di correttezza e completezza del medesimo, alcuni rudimenti di teoria dei modelli, le basi della teoria della computabilità, e i famosi teoremi di incompletezza di Gödel}.\\
...
assuefatto all'angustia di questo particolare vicolo cieco, mentre potrebbe esistere un calcolo logico di concezione completamente diversa e immune all'anatema di Gödel, se solo lo cercassimo con mente aperta? \purple{No, non c'è via di fuga}, 
...
\purple{Non anche se voi vi credete assolti, a patto che vi importi delle equazioni diofantee, siete coinvolti.} È infatti possibile rafforzare il corollario precedente.

\begin{theorem}
    [\href{https://en.wikipedia.org/wiki/Martin_Davis_(mathematician)}{\purple{Davis}} - \href{https://en.wikipedia.org/wiki/Hilary_Putnam}{\purple{Putnam}} - \href{https://en.wikipedia.org/wiki/Julia_Robinson}{\purple{Robinson}} (1960) + \href{https://en.wikipedia.org/wiki/Yuri_Matiyasevich}{\purple{Matijasevic}} (1970)]
    L'insieme dei polinomi $p(x_1,\dots,x_n)$ a coefficienti interi tali che l'equazione:
    \[ p(x_1,\dots,x_n) = 0\]
    abbia soluzione intera, non è calcolabile.
\end{theorem}

In altri termini, è inconcepibile una procedura - sia essa un sistema assiomatico o qualunque altro tipo di algoritmo - che, ricevendo in input il polinomio $p$, determina infallibilmente se l'equazione diofantea $p(x_1,\dots,x_n) = 0$ ha soluzione intera.
...
formula aritmetica, per dimostrare questo teorema occorre compilare ogni funzione computabile in un \purple{polinomio}.\\
...
il condannato. Le esecuzioni, lo sanno tutti, si fanno solo al mattino. Intanto - lemma 1 - l'esecuzione non può avvenire di sabato, altrimenti, venerdì pomeriggio, il condannato potrebbe conoscere il giorno con certezza. Quindi venerdì è l'ultimo giorno utile.
Per cui - lemma 2 - l'esecuzione non può avvenire venerdì, altrimenti, giovedì pomeriggio etc. Si vede bene che ogni giorno può essere escluso, quindi la sentenza è contraddittoria, e non potrà essere eseguita. Mercoledì mattina, però, il boia (assassino) taglia la testa - o lo avvelena,
...
in seria considerazione la possibilità di formalizzare non solo in teoria, \purple{ma in pratica}, la matematica per mezzo di opportuni sistemi informatici.
