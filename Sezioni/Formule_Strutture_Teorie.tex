...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item [$\boxed{\text{$L$-formule}}$] Sia ora \textcolor{purple}{$\varphi = \psi_1 \land \psi_2$} (o \textcolor{purple}{$\varphi = \psi_1 \lor \psi_2$} o \textcolor{purple}{$\varphi = \psi_1 \to \psi_2$}), allora $\vl(\varphi) = \vl(\psi_1) \cup \vl(\psi_2)$, segue che $v_1|_{\vl(\psi_i)} = v_2|_{\vl(\psi_i)}$, per cui per ipotesi induttiva $M \models \{v_1\} \psi_i \iff M \models \{v_2\} \psi_i$, allora per definizione di soddisfacibilità nella semantica di Tarski:
            ...
            Sia ora \textcolor{purple}{$\varphi = \forall x_k \; \psi$}, allora $\vl(\varphi) = \vl(\psi) \setminus \{x_k\}$, segue dall'ipotesi che $v_1|_{\vl(\psi)\setminus\{x_k\}} = v_2|_{\vl(\psi)\setminus\{x_k\}}$; usando la semantica di Tarski si ottiene:
            \begin{align*}
            ...
            Sia infine \textcolor{purple}{$\varphi = \exists x_k \; \psi$}, ho che $\vl(\varphi) = \vl(\psi) \setminus \{x_k\}$, segue che $v_1|_{\vl(\psi)\setminus\{x_k\}} = v_2|_{\vl(\psi)\setminus\{x_k\}}$, come prima, usando la semantica di Tarski si ottiene:
            \begin{align*}
...
\begin{soln}
    Sia $M = (D;i)$ un modello e $\psi$ una $L$-formula, vogliamo dimostrare che $M \models \psi$ se e solo se $M \models \textcolor{purple}{\forall x_k} \; \psi$. Per la semantica di Tarski, vale che:
    \[ M \models \textcolor{purple}{\forall x_k} \; \psi \iff \forall v : \Var \to D \; \forall a \in D \; M \models \{v[a/x_k]\} \psi
    \]
...
\begin{remark}[Caratterizzazione della coerenza]
    $T$ è coerente se e solo se $T \not \models \bot$\footnote{Con questa notazione si intende che esiste un modello $M$ di $T$ tale che $M \not\models \bot$, dove la notazione \textcolor{MidnightBlue}{$M \not \models \varphi$} è 
    un'abbreviazione per $\lnot (M \models \varphi)$, i.e. $\exists v : \Var \to D$ tale che $M \not\models\{v\} \varphi \overset{\text{def}}{\iff} \neg \; M \models\{v\} \varphi$, che per la semantica di Tarski è equivalente a $M \models\{v\} \neg \varphi$.}.
\end{remark}
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        cioè $M \models \{v\}\bot$ (come proposizione insiemistica nella meta teoria è) vera per ogni valutazione $v$. Ma $M \models\{v\} \bot$ è sempre falso, indipendentemente da $v$, per la definizione data dalla semantica di Tarski ($\neg\,M \models\{v\}\bot$ è un enunciato sempre vero in $\mathsf{ZFC}$), dunque per un'interpretazione qualsiasi si ha che $M \models\{v\} \bot$ è falso, per cui ho un assurdo.
...
\begin{definition}[Teoria completa di una struttura]
    Data una $L$-struttura $M$, definiamo la \vocab{teoria completa di $M$}, denotata $\Th(M)$ è l'insieme di tutte le $L$-formule \textcolor{purple}{$\boxed{\text{chiuse}}$} $\varphi$ tali che $M \models \varphi$.
\end{definition}
...
\begin{remark}[$L$-termini e morfismi di strutture]
    ...
    \begin{itemize}
        ...
        \item Se $t = f(t_1,\ldots,t_{\ar(f)})$, allora $F(\{v\}_M t) = F(f_M(\{v\}_M t_1,\ldots,\{v\}_M t_{\ar(f)})) = f_{M'}(F(\{v\}_M t_1),\ldots,F(\{v\}_M t_{\ar(f)})) = f_{M'}(\{F \circ v\}_{M'} t_1,\ldots,\{F \circ v\}_{M'} t_{\ar(f)}) = \{F \circ v\}_{M'} t$.\footnote{Insomma ho una sorta di pushforward della valutazione $v$ tramite $F$, $v_* := F \circ v$.}
    \end{itemize}
\end{remark}
